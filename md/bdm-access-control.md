...
Business objects used in a **composition relationship** are handled differently. Access control rules must be defined at its parent level, because a "child" has the same life cycle than its father. Since there is no access rule to define for a business object used in a composition relation, they are not displayed in the list of BO in the access control graphical editor, on the left, but they appear in the parent object list of attributes, in the details of the rule.
