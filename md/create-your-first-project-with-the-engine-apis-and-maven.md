...
In order to execute this client example you need to set the Java system property `bonita.home`, for example `-Dbonita.home=<path to bonita home>`, and [configure a bonita_home for the client](configure-client-of-bonita-bpm-engine.md). The bonita home used must have the same version as
