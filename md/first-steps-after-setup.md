...
Review the [REST API authorization](/rest-api-authorization.html) and the other features relating to [security and authentication](/security-and-authentication.html) that are available in Bonita BPM and in your operating system, and update your platform as required for your production environment. 
For example, most environments require a [password policy](/enforce-password-policy.html).
...
7. Create [users with the standard profile](/manage-a-user.html).
...
If you use an LDAP or Active Directory system, you can use the [LDAP synchronizer](/ldap-synchronizer.html) tool to keep the Bonita BPM Portal organization synchronized with it.