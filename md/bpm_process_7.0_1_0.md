...
Manage a [process definition](/key-concepts.html#process_definition) (not to be confused with a [case](#case), which is a process instance).
...
A process resource is created using the content of a `.bar` file that has previously been [uploaded](/manage-files-using-upload-servlet-and-rest-api.html), using the `processUpload` servlet `http://.../bonita/portal/processUpload`,
...
* [Standard search parameters](/web-rest-api-details.html#search) **p** and **c** must be specified
...
The process design object. The JSON returned is a representation of the [DesignProcessDefinition](/javadoc.html) instance of the given process id.
