...
3. MySQL must be configured with innoDB storage engine. Even if the 5.5.x line is recommanded, tests have shown that Bonita BPM  can run with MySQL 5.6.x and 5.7.x (please make tests prior using these versions, especially when using exotic encoding/charset). Notice that Bonita BPM does not work with MySQL 8.x
