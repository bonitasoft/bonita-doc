...
| Workspace API  | The Workspace API is provided to generate `.bar` files from the command line, ready for production. .BAR files are for deployment only and cannot be read by the Studio. .BOS files are for Studio interchange only and cannot be deployed to production. Each file format is optimal in term of content and is optimized for the target use case  | The Workspace API behaviour does not change, and can export pages that are mapped to process instantiation, process overview, or to tasks. It cannot export application pages that are not elements of a process. The Workspace API has been deprecated in 7.7.0, and replaced by the *LA builder* included in the tooling suite of [*Bonita Continuous Delivery* add-on](https://documentation.bonitasoft.com/bcd/2.0/). See [Automating builds](automating-builds.md)  |

## Feature improvements in Bonita Studio

| Feature  | Bonita  6.x  | Bonita 7.x  |
| -------- | --------------- | --------------- |
| UI designer  | Not in 6.x  | A new development environment for creating sophisticated, data-driven pages and forms for process-based applications. The UI designer is accessed through the Studio. See [live update](live-update.md). The 6.x forms are supported up to the 7.7.4, so that you can continue to run 6.x processes and to ease migration  |
| Process migration  | N/A| A process created in Bonita 6.x will run in 7.x (up to 7.7.4) without any modification. However, to take advantage of the new features in Bonita 7.x, you need to add contracts and migrate the forms. See [Contracts](contracts-and-contexts.md) and [Migrate a form from 6.x](migrate-a-form-from-6-x.md)  |
...
| Subtasks  | A Subtask is a part of a self-assigned existing task. A Subtask must be assigned to a specific person, by name. The assignee can be the creator  | Supported up to 7.2.4. No more substasks can be created starting from 7.3.0, but ongoing process instances should execute themselves correctly. See [Manage a subtask](subtasks.md)  |
