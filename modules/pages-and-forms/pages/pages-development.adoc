...
you need to specify the xref:ROOT:rest-api-authorization.adoc[REST API authorizations] that a user needs to have in order to access the resources in the custom page.
...
When a user is given access to this page because they are in a profile that contains it or that is mapped to an application that contains the page, then this user is granted, upon login, the permissions associated to these resources (see xref:ROOT:rest-api-authorization.adoc[REST API authorizations] for more details).
...
To enable custom page debug mode, edit xref:ROOT:BonitaBPM_platform_setup.adoc[`console-config.properties`] and set `custom.page.debug` to `true`.
...
If your page is viewed in an application, you will have access facilities for xref:ROOT:applications.adoc[the application theme].
