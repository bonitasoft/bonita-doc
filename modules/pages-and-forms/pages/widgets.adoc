...
[WARNING]
====
If you're using this widget in a digitally accessible applications, you could face some limitations (ex: the `input` tag doesn't have a linked label).
====
...
--
[.symptom]
My custom styling on pbChecklist or pbRadiobuttons isn't working as in oldest version.

[.symptom-description]
My custom css rule, from a theme or a css file is not apply after the migration on {bonitaVersion}.

[.cause]#Potential cause#
This two widgets was updated to following some accessibility rules so the internal HTML templating has been changed.

[.solution]#Possible solution#
Check you custom css selector, as example, <label> html tag is replaced by <legend>. For more details you can go on UI-Designer, Editor > Help (in the top right corner) > Migrations.
--
