...
It most of the times uses xref:ROOT:forms.adoc[forms] to display and collect new or edited information. +
...
The overall graphical structure of the application is handled by the xref:ROOT:layout-development.adoc[layout]. +
...
Developers can xref:ROOT:customize-living-application-theme.adoc[create a custom theme] using their favorite development environment, and then add it as an xref:software-extensibility.adoc[extension] in the project. +

Bonita UI Designer comes with a set of xref:ROOT:widgets.adoc[provided containers and widgets]. +
xref:ROOT:custom-widgets.adoc[Custom widgets] can also be created to make the palette richer. +
...
In a Bonita project architecture, xref:ROOT:forms.adoc[forms] are embedded in the pools. Their creation is very linked to the process step they "feed", in a way. +
It is recommended to define the xref:ROOT:contracts-and-contexts.adoc[contract] in its _Execution / Contract_ property tab before to create the form, so the Studio will generate a form according to the contract. +
...
For more technical information about layouts, visit the xref:ROOT:layout-development.adoc[dedicated documentation page].
