...
The call of an External API variable is triggered at page load, and every time the URL of the resource it points at changes.
...
For instance, to retrieve a task when the variable `taskId` is set:
...
----
if ($data.taskId) {
    return "../API/bpm/humanTask/{{taskId}}";
}
----
