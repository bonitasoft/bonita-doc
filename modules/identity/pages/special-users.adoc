...
Each tenant has an administrator (also known as the *tenant technical user*) with a tenant-specific username and password. The tenant administrator can manage maintenance mode for the tenant, install a Business Data Model, install the resources and the organization. +
