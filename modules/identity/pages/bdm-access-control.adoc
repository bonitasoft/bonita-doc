...
To grant access to specific instances of business objects, you will need to use xref:ROOT:rest-api-authorization.adoc[Rest api authorizations].
...
For that reason, Bonita takes advantage of the capability to define xref:ROOT:profiles-overview.adoc[profiles] in Bonita Studio. +
...
* Check that all users are also mapped with the default profile _User_.
* Define three custom profiles using the xref:ROOT:profileCreation.adoc[profile editor] in the Studio:
...
. Run _setup pull_. For more details on what this command does, see xref:ROOT:bonita-platform-setup.adoc[Bonita Platform Setup].
...
For more information about dynamic security and how it works with Bonita, see xref:ROOT:rest-api-authorization.adoc[Rest API authorization].
