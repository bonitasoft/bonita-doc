...
NOTE: The POST request must be a standard HTML form submission, not a request handled in javascript, otherwise the Browser will block the response due to CORS policy if the custom login page has a different origin (scheme, host and port) as Bonita webapp.
