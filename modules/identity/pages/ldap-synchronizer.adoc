...
*Cause:* +
The LDAP Synchronizer is using the Engine HTTP API of Bonita but it is not active in your configuration.

*Solution for on-premise bundle types of installations:* +

. Configure the .zip bundle you have downloaded from the Customer Service Center
. Edit the `/server/webapps/bonita/WEB-INF/web.xml` file 
...
*Solution for Docker image types of installations:* +
Run the docker image with *-e HTTP_API=true*
...
For more information, visit the xref:ROOT:bonita-docker-installation.adoc#_http_api[Deploy Bonita Runtime with Docker] documentation page.
