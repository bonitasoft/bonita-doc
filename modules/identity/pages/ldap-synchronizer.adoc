...
Configure the Bonita Tomcat bundle you have downloaded from the Bonitasoft Customer Service Center to activate
...
* For example: `docker run -e HTTP_API=true -e REST_API_DYN_AUTH_CHECKS=false --name bonita -v ./bonita-lic/:/opt/bonita_lic/ -h localhost -d -p 8080:8080 bonitasoft.jfrog.io/docker/bonita-subscription:pass:a[{bonitaVersion}]`
