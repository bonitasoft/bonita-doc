...
To troubleshoot Account Provisioning With issues, you need to increase the xref:ROOT:logging.adoc[log level] to `TRACE` for the packages `com.bonitasoft.engine.authentication` in order for errors to be displayed in the log files `bonita.log` (by default, they are not).
...
An `INFO` message is logged for each user, group, role or membership creation. Other `WARNING`, `DEBUG` and `TRACE` messages are logged when groups/roles already exist or if their creation fails.
