...
The JAAS configuration can include one or several login contexts. The Bonita login context must be named `BonitaAuthentication`.
...
[source,log]
----
c.b.e.a.i.JAASGenericAuthenticationServiceImpl - error on context creation :javax.security.auth.login.LoginException: No LoginModules configured for BonitaAuthentication
----
...
* The jaas configuration file does not contain any module named _BonitaAuthentication_.
...
* Make sure the definition of the LoginModules in your jaas configuration file respects the naming convention: the Bonita login context must be named `BonitaAuthentication. (See <<loginContextName, Login Context Name>> section)
--
