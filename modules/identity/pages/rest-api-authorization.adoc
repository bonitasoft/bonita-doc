...
* set the Environment variable `COM_BONITASOFT_ENGINE_AUTHORIZATION_REST_DYNAMIC_CHECK_ENABLE=false`
* or add the Java System property `-Dcom.bonitasoft.engine.authorization.rest.dynamic.check.enable=false` to your `setEnv[.sh|.bat]` tomcat startup script.
