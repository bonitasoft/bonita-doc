...
The default versions of these files are located in `setup/platform_conf/initial/tenant_portal`.
...
retrieve the current configuration and update the files in `setup/platform_conf/current/tenant_portal`.
Then use the tool again to save your changes into the database.

[discrete]
==== Resources permissions mapping
[#resources_permissions_mapping]
...
This specifies the REST API permissions that are granted by the Bonita Administrator User List page that lists all the users.
...
For some API resources, the static authorization check may not be specific enough to properly define when the access is granted. In those situations, the static authorization check may be overridden using dynamic checks.
...
* add the script to the `setup/platform_conf/initial/tenant_security_scripts` folder
...
* add the script to the `setup/platform_conf/current/tenant_security_scripts` folder
...
* make sure this .groovy file is placed in the default directory, under 'initial/tenant_security_scripts/' if the platform has never been started,
or under 'current/tenant_security_scripts/' if the platform has already been started
...
class CasePermissionRule implements PermissionRule {
    ...
    boolean isAllowed(APISession apiSession, APICallContext apiCallContext, APIAccessor apiAccessor, Logger logger) {
        ...
        if ("GET" == apiCallContext.getMethod()) {
            return checkGetMethod(apiCallContext, apiAccessor, currentUserId, logger)
        } else if ("POST" == apiCallContext.getMethod()) {
    ...
    private boolean checkGetMethod(APICallContext apiCallContext, APIAccessor apiAccessor, long currentUserId, Logger logger) {
        ...
        if (apiCallContext.getResourceId() != null) {
            ...
            if (stringUserId == filters.get("started_by") || stringUserId == filters.get("user_id") || stringUserId == filters.get("supervisor_id")) {
