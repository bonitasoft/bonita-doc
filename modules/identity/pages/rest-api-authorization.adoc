...
In order to change the configuration on an installation whose platform has already been initialized, use the xref:ROOT:bonita-platform-setup.adoc[platform setup tool] to
...
* use the xref:ROOT:bonita-platform-setup.adoc[platform setup tool] to retrieve the current configuration
* add the script to the `setup/platform_conf/current/tenants/[tenantId]/tenant_security_scripts` folder
* then use the xref:ROOT:bonita-platform-setup.adoc[platform setup tool] again to push the new / modified scripts to database
...
When a user logs in, after the user is xref:ROOT:user-authentication-overview.adoc[authenticated], a map of `LoggedUserPermissions` is created.
...
If the page declares resources provided by a xref:ROOT:rest-api-extensions.adoc[REST API extension], then the REST API extension must be deployed before the page,
...
When a new xref:ROOT:custom-profiles.adoc[custom profile] is created, the permissions mappings are updated in the configuration files and in the cache.
...
For more details see : xref:ROOT:bdm-access-control.adoc[BDM access control]
...
. Export the xref:ROOT:pages.adoc[custom page].
. Update the page properties with xref:ROOT:pages.adoc[permissions].
...
. Import the xref:ROOT:pages.adoc[custom page].
...
You can find the rest API authorizations xref:default-api-call-permissions-overview.adoc[in this page].