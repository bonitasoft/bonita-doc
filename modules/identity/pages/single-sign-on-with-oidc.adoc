...
[discrete]
==== Setup Bonita Runtime for OIDC logout
...
If the logout flow supported by your OIDC provider is not the same as the one supported by Bonita Runtime, the preferred solution to handle it anyway is to intercept the requests to /logoutService and handle the logout programmatically.
====
