...
+
When accounts creation on the fly once a user accessing Bonita has been authenticated with the IdP is required, you can uncomment the property `authentication.passphraseOrPasswordAuthenticationService.createMissingUser.enable` (as well as subsequent properties) and set its value to true. For more information see xref:identity:user-account-provisioning-with-sso.adoc[User account provisioning with single sign-on (SSO)].
+
