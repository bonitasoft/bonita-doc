...
In your OIDC identity provider (IdP) configuration, you should make sure the `bonita` OIDC Service Provider declares as `Allowed CORS web origins` the URLs of the domains where the pages requiring calls to Bonita REST API are hosted (or `*` if you want to allow origins).
