 ...
 ** Some additional properties no present in the default file can be useful:
  *** `discovery-url` can be used if the OIDC provider provides a discovery URL for OIDC configuration (".well-known") to replace the 6 properties `auth-url`, `token-url`, `logout-url`, `token-issuer-uri`, `user-info-url` and `jwks-url`. This property is specific to Bonita OIDC module and is not supported/documented by Keycloak.
