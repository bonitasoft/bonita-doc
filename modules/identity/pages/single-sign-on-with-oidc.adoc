 ...
 ** Some additional properties not present in the default file can be useful:
  ...
  *** `confidential-port` can be used to force the port in the redirect-uri sent to the OIDC provider when exchanging a code for tokens (setting the vallue of this property to 443 when `ssl-required` is set to `all` will result to the port not being added to the redirect-uri. This can help to avoid having a port wrongly added to the redirect-uri)
