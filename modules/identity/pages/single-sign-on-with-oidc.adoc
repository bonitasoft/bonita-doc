...
In the file *keycloak-oidc.json*, Make sure the property `enable-cors` is set to true and the properties `cors-allowed-methods`, `cors-allowed-headers`, `cors-exposed-headers` have the correct values. For a basic configuration, you can use the same values as in the xref:security:enable-cors-in-tomcat-bundle.adoc#_add_cors_filter[configuration of the CORS filter] (which is the way to handle CORS when Bonita web application is not configured for Authentication with OIDC). Also make sure to update the xref:security:enable-cors-in-tomcat-bundle.adoc#_choose_cookies_samesite_policy[sameSiteCookies policy] of the Tomcat server.
