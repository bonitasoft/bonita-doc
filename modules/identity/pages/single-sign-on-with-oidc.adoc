...
Make sure the property `autodetect-bearer-only` is set to `true` in the file *keycloak-oidc.json*. This property allows Bonita OIDC filter to detect if the request is a REST API call or a navigation call based on the headers of the request. In order for the Bonita REST API request to be authorized with the Bearer token, one of the following assertion must be true:
