 ...
 ** Some additional properties no present in the default file can be useful:
  *** `discovery-url` if the OIDC provider supports the OIDC discovery specification, this property can be used to configure the discovery endpoint URL (".well-known") to retrieve the provider's endpoint locations. It replaces the 6 properties `auth-url`, `token-url`, `logout-url`, `token-issuer-uri`, `user-info-url` and `jwks-url`. This property is specific to Bonita OIDC module and is not supported/documented by Keycloak.
