...
public class CustomAuthorizationRuleMapping implements AuthorizationRuleMapping {
    ...
    public List<String> getProcessStartRuleKeys() {
        return List.of("CUSTOM_RULE_UNIQUE_ID");
    }
...
* add customRule bean registration in `platform_conf/current/tenant_engine/bonita-tenants-custom.xml`
...
* add customAuthorizationRuleMapping bean registration in `platform_conf/current/tenant_engine/bonita-tenants-custom.xml`
...
* uncomment to declare customAuthorizationRuleMapping in `platform_conf/current/tenant_engine/bonita-tenant-community-custom.properties`
