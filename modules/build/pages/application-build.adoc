...
:description: How to build a Bonita self-contained application from a Bonita project.
...
** Maven Central repositroy
** xref:software-extensibility:bonita-repository-access.adoc#_maven_settings_overview_outside_bonita_studio[Bonita Artifact Repository] (for building projects using Enterprise edition)

## Maven build

Since 2023.2, Bonita project leverage Maven as a build tool. It uses an https://bonitasoft.github.io/bonita-project-maven-plugin/[in-house maven plugin] to build the Bonita specific artifacts like process digrams, UID pages and so on.  
...
[source,shell]
----
$ my-project> mvn package -Pbundle
----
...
By default, the `bonita.environment` is set to `local`. To override this value, you may add the `-Dbonita.environment=production` system property to the above command, where `production` is an existing enviroment in the project.

.Build using the docker packaging
[source,shell]
----
$ my-project> mvn package -Pdocker
----
...
[NOTE]
--
When building an enterprise edition project, make sure to be xref:software-extensibility:bonita-repository-access.adoc#_configure_access_to_bonita_artifact_repository_with_docker[logged in] to the `bonitasoft.jfrog.io` registry first.
--
...
* `-Ddocker.baseImageRepository`: `bonita` by default. When creating a projec with enterprise edition, this property is overriden to `bonitasoft.jfrog.io/docker/bonita-subscription`.
...
When at least one of the processes from a project contains a parameter, a Bonita configration file (.bconf) is generated during the build. This archive stores the parameters values and is specific to a given environment.  

### Extract parameters configuration

It is possible to extract the processes configurations into a single `yaml` using the following maven:
...
It is possible to override an exsting file using the `-Dparameters.overwrite` system property.
...
You may have the same parameter name in more than one processes and you want to override them in all processes, to do that you can edit the `parameters-${bonita.environment}.yml` file as shown:
