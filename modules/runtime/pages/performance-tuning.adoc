...
The work service is responsible for asynchronously processing execution of process instances. The work service has its own thread pool, which is configurable. +
...
The work service is configured in `bonita-tenant-community-custom.properties` and `bonita-tenant-sp-custom.properties` (cf xref:bonita-platform-setup.adoc[platform setup]).
...
The *work execution delay* is configured in xref:runtime:bonita-platform-setup.adoc[`bonita-tenant-community-custom.properties`] for postgres and xref:runtime:bonita-platform-setup.adoc[`bonita-tenant-sp-custom.properties`] for other supported DBMS.
...
The connector service executes connectors. To improve isolation (and to protect against denial-of-service attacks), the default implementation of the connector service has its own thread pool and executes connectors in a separate thread from the worker. +
...
In Bonita, this L2 Cache is implemented using the https://docs.hazelcast.com/hazelcast/latest[Hazelcast] library.
Hazelcast configuration for this persistence layer is defined in a file named `hazelcast.xml`. +
...
* Hibernate metrics are available from the API route `https://<BONITA_URL>/bonita/metrics` (see xref:runtime:runtime-monitoring.adoc#prometheus-publisher[Prometheus Publisher] for more details on how to retrieve those metrics)
...
* Use xref:runtime:bonita-platform-setup.adoc[setup tool] to update your configuration file accordingly. +
