...
When a flownode fails, the exception that caused the failure is stored and can be retrieved by the API (Enterprise only). A failure is annotated with additional context information and can also be consulted after the flownode has been archived.
