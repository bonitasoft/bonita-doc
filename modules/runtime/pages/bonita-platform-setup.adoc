...
*Symptom*: +
When I run Bonita Setup Tool, I get the exception `Cannot determine database vendor (valid values are h2, postgres, sqlserver, oracle, mysql).`
...
. Ensure there is a valid `db.vendor` value. 
