...
It is located in the xref:ROOT:tomcat-bundle.adoc[Tomcat bundle]. You can find the tool in the `setup` folder.
...
If you have already run `start-bonita` script inside a xref:ROOT:tomcat-bundle.adoc#configuration[Tomcat bundle] , those steps are already done.
====
