...
It is also located in the xref:bonita-docker-installation.adoc[Docker image] of Bonita, in the `opt/bonita/setup/` folder.
...
It prepares the Runtime for a first start.
+
Eg. `setup.sh init` +
...
The platform-setup tool is available in 2 different editions:

* The Community edition (only supports PostgreSQL).
* The Subscription edition (supports PostgreSQL, MySQL, Oracle and SQL Server).
...
This information is used internally by the Bonita Setup tool to configure properly the bundle (See <<run_bundle_configure,configure command>>) and the database initialization procedure (See <<init_platform_conf,init command>>). +
