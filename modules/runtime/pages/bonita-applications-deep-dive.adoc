...
* Generic applications. They are fully functional, can be customized and can be deleted if not useful in a Production environment:
  ** Administrator
  ** User
* Bonita applications. They are developped by Bonitasoft and delivered as independent applications. You can download them from the Customer Portal/Customer Service Center and install by folowing the same way as custom applications: 
  ** Reporting App
