...
At this stage no organization information is loaded: only the technical administrator account exists.
...
. Edit the file: `/setup/platform_conf/current/tenant_portal/security-config.properties`
. Make sure this line is set to `true`: `security.rest.api.authorizations.check.enabled true`
. Then edit the file: `/setup/platform_conf/current/tenant_portal/dynamic-permissions-checks-custom.properties`
...
. Save the changes made in file  `/setup/platform_conf/current/tenant_portal/dynamic-permissions-checks-custom.properties`.
...
NOTE: do not create a user or an administrator with the same login and password as the technical accounts (platform admin or technical administrator)

. Log into the Bonita Runtime as the technical administrator. +
*Note:* If your system is using single sign-on with CAS, you need to log in with the following URL: `+http://hostname:port/bonita/login.jsp?redirectUrl=/bonita/apps/appDirectoryBonita/+`.
