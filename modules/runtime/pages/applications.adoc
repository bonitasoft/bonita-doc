...
* xref:pages.adoc[Pages] and xref:forms.adoc[Forms]: the user interface components of the application. An application must contain at least one page.
* xref:admin-application-process-list.adoc[Processes]: the logic execution components of the application. An application must contain at least one process. A process can contain xref:forms.adoc[forms], which are the user interface of the process.
* xref:data-handling-overview.adoc[Data]: the business information created, transformed, and/or deleted by the processes, and presented in pages and forms.
...
* The xref:bdm-management-in-bonita-applications.adoc[Business Data Model] (BDM) must be deployed
* The xref:organization-in-bonita-applications-overview.adoc[organization] data must be deployed, so that application users can log in.
...
So we strongly encourage your team to create applications in xref:applicationCreation.adoc[Bonita Studio].
...
To access the application content, you must pertain to the xref:profiles-applications-overview.adoc[profile] mapped with the application. +
