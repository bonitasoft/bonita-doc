...
With the `batch-delete` mode, although it depends on your database configuration and the volume of data to delete, we recommend starting with a batch size of `5000` rows. You can increase this value based on the performance of your database. +
With the `copy-truncate` mode, you can use a larger batch size to improve performance.
