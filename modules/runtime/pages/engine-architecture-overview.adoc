...
Folder  `setup/platform_conf/[initial|current]/tenant_engine` contains tenant-level template configuration files used when creating a new tenant (Subscription edition):
...
Folder  `setup/platform_conf/current/tenant_engine` where `[TENANT_ID]` is your tenant id (e.g. `1`). Contains same files use for an already existing tenant.
