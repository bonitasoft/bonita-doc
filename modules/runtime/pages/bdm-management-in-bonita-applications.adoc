...
Only one model can be deployed at a time in the Bonita Runtime, so make sure it contains the Business Object definitions used in all processes that will run.
...
. In the Bonita Runtime, log in as the technical administrator (default login 'install', default password 'install').
. Go to the *Platform maintenance* menu.
. Click on *Activate maintenance mode* button.
+
[NOTE]
====
...
. Log in as the technical administrator (default login 'install', default password 'install', or refer to the internal documentation if modified as it should be).
. Go to the _Platform maintenance_ menu.
. Click on the _Activate maintenance mode_ button.
...
The Business Data Model will now be installed. Maintenance mode can be disabled. +
 To do so:
. Go to the _Platform maintenance_ menu.
. Click on the _Deactivate maintenance mode_ button.
+
[WARNING]
====
...
. Stop the BPM and BDM services (by activating the maintenance mode)
...
. Restart your BPM and BDM services (by disabling the maintenance mode)
...
. In the Bonita Runtime, log in as the technical administrator (default login 'install', default password 'install') or as a user with the administrator profile.
. As the technical admin or as an administrator, go to the *BDM* menu.
...
[NOTE]
====
Contrary to the BDM definition, the deployment of a BDM Access Control file does not require activating the maintenance mode during the operation.
====
...
* The Java classes that map the new model are created / updated and loaded in the JVM classloader. This triggers the refreshing of the classloader of *all nodes of the cluster* automatically.
