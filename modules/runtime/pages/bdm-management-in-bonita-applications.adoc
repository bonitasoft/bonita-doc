...
. Click on *platform-maintenance-mode* to xref:ROOT:platform-maintenance-mode.adoc[enable] the maintenance mode.
+
[NOTE]
====
The deployment of a Business Data Model requires enabling maintenance mode during the operation, so that the update can be performed without affecting ongoing processes.
...
. Click on the _Start maintenance_ button to xref:ROOT:platform-maintenance-mode.adoc[enable maintenance mode].
...
The Business Data Model will now be installed. Maintenance can be ended +
 To do so:
. Go to the _Platform maintenance_ menu.
. Click on the _End maintenance_ button to xref:ROOT:platform-maintenance-mode.adoc[disable maintenance mode].
+
[WARNING]
====
...
[NOTE]
====
Contrary to the BDM definition, the deployment of a BDM Access Control file does not starting maintenance mode during the operation.
====
