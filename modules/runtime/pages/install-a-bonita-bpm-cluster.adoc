...
property only takes effect from Bonita 2025.2-u11 (xref:version-update:product-versioning.adoc#technical-id[technical id] 10.4.11).
Outside cluster mode a single node also starts a Hazelcast member, for its second-level cache (the member exists only
while `bonita.platform.persistence.use_second_level_cache` is `true`, its default); from 2025.2-u11 that member listens
on loopback only, so nothing has to be done for it. If a Hazelcast client or Management Center must reach it, set
`bonita.platform.hazelcast.interfaces.list` to an empty value in `bonita-platform-sp-custom.properties`, as
`bonita.platform.hazelcast.interfaces.list=` with nothing after the equals sign. Uncommenting the line as shipped in
that file keeps the loopback default.
...
. Edit file *`setup/database.properties`* and modify the properties to suit your databases (Bonita internal database & Business Data database). Beware of  xref:runtime:bonita-platform-setup.adoc#troubleshooting[backslash characters].
...
`bonita.platform.cluster.work.recoveryLockLeaseSeconds` are available from Bonita 2025.2-u11
(xref:version-update:product-versioning.adoc#technical-id[technical id] 10.4.11) onwards.
