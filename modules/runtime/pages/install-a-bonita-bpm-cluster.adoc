...
property only takes effect from Bonita 2026.1-u3 (xref:version-update:product-versioning.adoc#technical-id[technical id] 11.0.3).
Outside cluster mode a single node also starts a Hazelcast member, for its second-level cache (the member exists only
while `bonita.platform.persistence.use_second_level_cache` is `true`, its default); from 2026.1-u3 that member listens
...
`bonita.platform.cluster.work.recoveryLockLeaseSeconds` are available from Bonita 2026.1-u3
(xref:version-update:product-versioning.adoc#technical-id[technical id] 11.0.3) onwards.
