...
It is possible to choose between displaying only the root cases (this was the only option before Bonita 2025.1) and all the cases
 including the one started by call ativities.
...
The case details page displays general information about the case, as well as monitoring information showing the number of done, pending, and failed flow nodes for the case and its possible child cases (started by call activities).
...
In the case details page, when the case or a flow node of the case or a child case is failed, an additional "Error details" panel displays the information about the exception that caused the failure.
When available, the scope and context of the error is attached to the failure to help locating the issue in the process definition. If the failure happened on a flow node, a link to the flow node details page is provided.
A failure history is also displayed in case of successive failures in the case.
