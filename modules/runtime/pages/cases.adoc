...
Remember that a failure at a step does not necessarily mean that the case has failed entirely, but it might indicate that some action is needed, perhaps to xref:admin-application-task-list.adoc#_replay_a_failed_task[replay a connector].
