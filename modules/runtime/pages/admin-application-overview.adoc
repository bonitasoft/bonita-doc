...
This living application is not installed by default in the Bonita tomcat bundle and in Bonita docker image in application.footnote:adminApplication[In the deprecated platform mode, this living application still be embedded by default.]
...
But you can reach one of our Professional Services team member to help you develop or maintain it. You can do so on the {cscCustomerServices}[Customer Service Center].
====
