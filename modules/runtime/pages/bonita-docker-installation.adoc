...
When the container is up and ready, you can access Bonita at the address http://localhost:8080/bonita and log in using the xref:identity:special-users.adoc#technical-user[technical administrator].
...
[source,shell]
----
docker run --name bonita [...] -v ~/local-folder-path/:/opt/bonita/setup/platform_conf/initial/tenant_portal/ [...]
----
...
* https://hub.docker.com/r/bonitasoft/bonita-mysql[MySQL] (not supported with Community edition)
* https://hub.docker.com/r/bonitasoft/bonita-sqlserver[SQL Server] (not supported with Community edition)
...
This optional environment variable is used in conjunction with BONITA_RUNTIME_ADMIN_USERNAME to define the username for the technical administrator. If it is not specified, the default username `install` will be used.

=== BONITA_RUNTIME_ADMIN_PASSWORD

This environment variable is recommended for you to use the Bonita image. It sets the technical administrator password for Bonita. If it is not specified, the default password `install` will be used.
...
=== BONITA_RUNTIME_BUSINESS_DATA_SERIALIZATION_STANDARD_SHAPE_ENABLED
This optional environment variable controls whether BDM JSON serialization follows standard REST shapes or legacy array-based formats. +
When enabled (*true*): Count queries return `{ "value": n }` instead of `[n]`, and single-entity queries return `{ ... }` instead of `[ { ... } ]`. +
When disabled (*false*, default): Legacy array format is used for backward compatibility. +
Default value is *true*. +
See xref:data:bdm-query-response-formats.adoc[BDM Query Response Formats] for detailed information about response formats and migration guidance.
...
To ensure the below init scripts are available when the container is started or restarted, place them on the host system and mount a Docker volume on the containing directory.

=== `/opt/custom-config.d/` directory
...
All configuration scripts with the .sh suffix located in this directory are sourced at first container start.
