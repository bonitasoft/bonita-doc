...
On the other side, the database refactoring is xref:bdm-management-in-bonita-applications.adoc#_limitations[only partially supported].
...
See xref:bdm-management-in-bonita-applications.adoc#_manage_the_bdm[how to deploy the BDM through Bonita Super Administrator application].
