...
====
This timeout can be increased in the Connector service configuration in  `bonita-tenant-sp-custom.properties` (See xref:ROOT:BonitaBPM_platform_setup.adoc[platform setup]) by increasing the value of the property `bonita.tenant.connector.timeout`.  
