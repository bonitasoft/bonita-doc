...
Bonita also provides a {javadocUrl}/api/{javadocVersion}/index.html[PlatformMonitoringAPI] that you can use to obtain some of this information.
...
These parameters can be obtained through monitoring system like Nagios or Zabbix. You can also retrieve them using the {javadocUrl}/api/{javadocVersion}/index.html[PlatformMonitoringAPI].
...
You can also get the number of active transaction using the {javadocUrl}/api/{javadocVersion}/index.html[MonitoringAPI].
...
* Check connection number managed by Ofelia Server (See xref:ROOT:performance-tuning.adoc[Work Service], Connector service, and xref:performance-tuning.adoc[Scheduler Service])
* Check number of database connections defined in Ofelia (see xref:ROOT:performance-tuning.adoc[Database connections] and xref:performance-tuning.adoc[Datasource settings])
