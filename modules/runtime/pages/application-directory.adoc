...
[WARNING]
==
On development phase, if you try to access to a xref:applications:ui-builder/version-control-with-git/version-control-with-git.adoc[UI-Builder application connected to Git repository] from the Application Directory, the redirection will fail.

To access the UI Builder application, you need to access it directly from the URL (e.g `http://localhost:8090/`)
==
