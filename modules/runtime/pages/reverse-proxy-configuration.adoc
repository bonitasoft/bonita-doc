...
[WARNING]
====
Ofelia does not support anymore having Bonita Runtime running on a dedicated Tomcat server on one side, and Bonita Engine running on a dedicated Tomcat server on the other side.
====
...
image::tomcat-behind-reverse-proxy.png[Tomcat behind a firewall with reverse proxy]
