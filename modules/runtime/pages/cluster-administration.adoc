...
:description: Discover how to administer, monitor and manage a Bonita cluster.
...
In addition, there is an incident file <BUNDLE_HOME>/logs/incident.log (where [TENANT_ID] is the ID of the tenant on which the error occurred) that is used as a last resort to record an error that cannot be handled, such as failure to access the database.
--
