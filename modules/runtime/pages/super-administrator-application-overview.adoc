...
* Ability to notify users about a scheduled maintenance, by a custom message that will be displayed to logged-in users. this message is also displayed on the maintenance page when the maintenance mode is started.
* Start maintenance mode for operations, among which the update of the Business Data Model (and the related database migration)
