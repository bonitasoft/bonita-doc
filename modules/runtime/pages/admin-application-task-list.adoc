...
With the Access, Enterprise, Performance, and Efficiency editions, an Administrator can do a task for another user. +
...
[NOTE]
====
"Do for" is a one-off action on a single task. To let someone cover all of an absent user's assigned tasks over a period, set up a delegation instead. See xref:admin-delegation.adoc[Delegations administration].
====
...
This feature is available with the Access, Enterprise or the Performance editions.
