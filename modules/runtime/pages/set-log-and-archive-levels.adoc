...
You can xref:ROOT:logging.adoc[configure the log level]. The log level can be set on several categories in the `logging.properties` file. Some categories are very specific to a particular issue.
For example:
...
To configure what information you want to record in the quieriable log, modify the xref:ROOT:bonita-platform-setup.adoc[`bonita-tenant-sp-custom.properties`].
