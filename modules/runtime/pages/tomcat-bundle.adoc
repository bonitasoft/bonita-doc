...
The Tomcat bundle is a .zip archive that contains the Apache Tomcat Java EE application server prepackaged along with Bonita and xref:runtime:bonita-platform-setup.adoc#bonita_setup_tool[Bonita platform setup tool].
...
* The Tomcat bundle is no longer needed, thanks to our recommended deployment mode, which is described here: xref:build-run:build-application.adoc[how to build a new application].
...
As a security precaution, we *strongly recommend* that before you start your application server, you change the default username and password used for the platform administrator and for the default technical administrator.

==== Platform administrator

The username and password are defined in a text file:

* Before the very first Tomcat start in the `initial` folder: xref:runtime:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/initial/platform_engine/bonita-platform-community-custom.properties`]
* After the first Tomcat start in the `current` folder: xref:runtime:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/current/platform_engine/bonita-platform-community-custom.properties`]
...
This password is used for platform-level administration tasks, such as starting/stopping a node, retrieving the platform state...

==== Technical administrator

The Bonita platform has a technical administrator, with a specific username and password. The technical administrator was formerly known as the technical administrator.

When the platform is created, default values for the technical administrator username and password are defined in the file xref:runtime:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/initial/tenant_engine/bonita-tenant-community-custom.properties`], by the following properties:

* `bonita.runtime.admin.username` defines the username (default `install`)
* `bonita.runtime.admin.password` defines the password (default `install`)

You can change these credentials by updating these properties directly in the file.
