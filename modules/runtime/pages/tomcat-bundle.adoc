...
The Tomcat bundle is a .zip archive that contains the Apache Tomcat Java EE application server prepackaged along with Bonita and xref:ROOT:bonita-platform-setup.adoc#platform_setup_tool[Bonita platform setup tool].
...
Once initialized, to update this configuration, use the xref:ROOT:bonita-platform-setup.adoc[_Platform setup tool_] embedded in Bonita bundles.
====
...
* Before the very first Tomcat start: xref:ROOT:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/initial/platform_engine/bonita-platform-community-custom.properties`]
* After the first Tomcat start: xref:ROOT:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/current/platform_engine/bonita-platform-community-custom.properties`]
...
When the platform is created, default values for the tenant administrator username and password are defined in the file xref:ROOT:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/initial/tenant_template_engine/bonita-tenant-community-custom.properties`], by the following properties:
...
* Before the very first Tomcat start: xref:ROOT:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/initial/platform_portal/platform-tenant-config.properties`]
* After the first Tomcat start: xref:ROOT:bonita-platform-setup.adoc[`<BUNDLE_HOME>/setup/platform_conf/current/platform_portal/platform-tenant-config.properties`],
...
. Edit file `<BUNDLE_HOME>/setup/database.properties` and modify the properties to suit your databases (Bonita internal database & Business Data database). Beware of xref:ROOT:bonita-platform-setup.adoc#backslash_support[backslash characters].
...
For advanced server configuration needs: check out xref:ROOT:bonita-platform-setup.adoc#run_bundle_configure[Bundle configuration] to finely tune your Tomcat bundle, using templates suggested by Bonita.
====
...
To update Bonita configuration after the first run, take a look at the xref:ROOT:bonita-platform-setup.adoc#update_platform_conf[platform setup tool]

[NOTE]
====


* The file `database.properties` is the entry point to configure the xref:ROOT:bonita-platform-setup.adoc#run_bundle_configure[Tomcat environment] and the xref:bonita-platform-setup.adoc#configure_tool[Bonita Platform configuration].
...
To update the licenses after the first run, take a look at the xref:ROOT:bonita-platform-setup.adoc#update_platform_conf[platform setup tool]
