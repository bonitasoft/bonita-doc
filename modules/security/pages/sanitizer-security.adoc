...
This exclusion is provided by Bonita, is active out of the box, and requires no
configuration. In earlier versions it is not present: add it yourself, as shown below, if you use that page.
...
The Sanitizer protection can also be disabled per JSON attribute, independently of the URL.
...
If you want to disable the Sanitizer protection for a specific json attribute, edit the `setup/platform_conf/initial/platform_portal/security-config.properties` configuration file and append your attribute name to the `security.sanitizer.exclude` comma-separated values.

This mechanism matches on attribute names and applies across the whole REST API.
