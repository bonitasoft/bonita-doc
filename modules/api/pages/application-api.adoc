= Application API
:page-aliases: ROOT:application-api.adoc
:description: Bonita application API allows you to retrieve the current deployed application definitions
...
`GET`
* *URL Params* +
xref:ROOT:rest-api-overview.adoc#resource_search[Standard search parameters] are available to search form mappings. +
*Required*
 ...
 ** f: can filter on "token", "displayName", "version", "profileId", "creationDate", "createdBy", "updatedBy", "lastUpdateDate" with the format `+f={filter_name}={filter_value}+`, "userId" to get the applications for a specific user
