...
See xref:ROOT:rest-api-authentication.adoc[REST Authentication] details to understand how to login from Bonita REST APIs.
...
. xref:ROOT:bpm-api.adoc#case[Start a new case with variables]: Provide a form for the user to enter initial data. Then call the method to start a new case using the values entered by the user to initialize some variables. The engine will start the execution of the process. Depending on the design of your process, there might then be some human tasks available for the end user.
. xref:ROOT:bpm-api.adoc#human-task[List the pending tasks for a user]: Retrieve a list of available human tasks for the logged in user. When the user selects a task to do, you can display the corresponding form. It can be an external form or a Bonita form that can be accessed by url.
. xref:ROOT:bpm-api.adoc#activity[Update variables and execute a task]: If your application is using an external form, update the values of the variables in your process.
...
. xref:ROOT:bpm-api.adoc#connector-instance[Handle tasks in error]: Get a list of tasks that are in the failed state, and then replay each task by doing three steps: get the list of failed connectors, reset the state of failed connectors and replay the failed task.

=== Logout from Bonita

When processing is complete, you must log out.

See xref:ROOT:rest-api-authentication.adoc[REST Authentication] details to understand how to logout from Bonita REST APIs.

== API Extensions

You can create xref:api:rest-api-extensions.adoc[Rest API Extensions] to extend the Rest API by adding missing ressources (not provided by the Rest API).
...
If you are searching for business data using a custom query, there must be a xref:data:define-and-deploy-the-bdm.adoc[count query in the BDM]. If there is no count query, results from a custom query on business data cannot be paged properly (the header Content-Range will be absent).
...
* s: search on name or search keys. The matching policy depends on the configuration of xref:ROOT:using-list-and-search-methods.adoc[word-based search].
...
* An open source https://github.com/bonitasoft/bonita-java-client[java client] is implemented above the HTTP API. It is available on https://search.maven.org/search?q=g:%22org.bonitasoft.web%22%20AND%20a:%22bonita-java-client%22[Maven central].
* An https://www.openapis.org/[OpenAPI specification] is also available as a `yaml` descriptor or a `json` https://www.postman.com/collection/[Postman collection] https://github.com/bonitasoft/bonita-openapi/releases[here].
