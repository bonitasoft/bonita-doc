...
This way of creating a case using this method will only work for processes in which no contract is defined. To instantiate a process with a contract, check the xref:api:actors-process-connectors-api.adoc#start-a-process-using-an-instantiation-contract[process instantiation resource documentation].
...
See xref:api:actors-process-connectors-api.adoc#start-a-process-using-an-instantiation-contract[Start a process using an instantiation contract] for usage.
====
...
=== CaseInfo (Subscription editions only)
 ...
 ** *Code*: 200
 ** *Payload*: +
In this example, counters are returned for two flow nodes in the case, Step1 and Step3. For Step3, there are no flow nodes in ready state, so no counter is returned for this. Possible states keys are: `ready`, `executing`, `waiting`, `initializing`, `failed`, `completing`, `completed`, `skipped`, `cancelled`, `aborted`.
+
[source,json]
----
{
