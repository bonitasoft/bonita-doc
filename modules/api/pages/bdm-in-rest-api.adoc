...
Learn best practices when developing a xref:ROOT:rest-api-extensions.adoc[REST API Extension] accessing xref:define-and-deploy-the-bdm.adoc[Business Data]. +
...
Note that the relationships from car to wheel1, wheel2, wheel3, wheel4 are *xref:ROOT:define-and-deploy-the-bdm.adoc#lazy_eager_loading[lazy] loaded* (in the Studio, it is
...
* If lazy loaded objects should always be returned in the response, consider changing the relation from xref:ROOT:define-and-deploy-the-bdm.adoc#lazy_eager_loading['lazy' to 'eager']
in the model.
====
