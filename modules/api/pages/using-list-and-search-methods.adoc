...
List<ArchivedActivityInstance> archivedActivityInstances =
    apiClient.getProcessAPI().getArchivedActivityInstances(instance.getId(), 0, 100, ActivityInstanceCriterion.NAME_ASC);
----
...
SearchResult<ArchivedActivityInstance> archActivityResult = apiClient.getProcessAPI().searchArchivedActivities(searchBuilder.done());
----
...
you can have an OR clause, of more complex clauses. See {javadocUrl}/api/{javadocVersion}/org/bonitasoft/engine/search/SearchOptionsBuilder.html[SearchOptionsBuilder methods] for filtering that matches your needs. +
