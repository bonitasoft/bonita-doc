...
:page-aliases: ROOT:handle-a-failed-activity.adoc
...
https://javadoc.bonitasoft.com/api/{javadocVersion}/index.html[Javadoc]. These methods are accessed through the ProcessAPI, which extends the ProcessManagementAPI.

First you need to log in and get the ProcessAPI:
...
----
        ...
        for (Log log : searchedLogs.getResult()) {
            // Print the failed connector reason message:
