...
----
+
No business users are created by default and the default username/password for the technical user to connect to Bonita Engine can be retrieved in the file `setup/platform_conf/current/tenant_engine/bonita-tenant-community-custom.properties` using the xref:runtime:bonita-platform-setup.adoc[platform setup tool].
+
