...
+
This will create and initialize Bonita Engine environment.
After this step, the technical administrator will be able to connect to the engine and create new users or xref:ROOT:manage-an-organization.adoc[import the organization].
+
[source,bash]
----
// Initialize the platform
...
----
+
No business users are created by default and the default username/password for the technical administrator to connect to Bonita Engine can be retrieved in the file `setup/platform_conf/current/tenant_engine/bonita-tenant-community-custom.properties` using the xref:runtime:bonita-platform-setup.adoc[platform setup tool].
+
