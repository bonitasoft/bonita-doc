...
In order to make Bonita applications available to end users you need to set up a Bonita Runtime. Please refer to the Bonita Runtime xref:runtime:runtime-index.adoc[installation section]. If you have to expose Bonita Runtime or a Bonita-developed application publicly, but legitimately want to protect any other resources behind your xref:runtime:reverse-proxy-configuration.adoc[firewall] 
