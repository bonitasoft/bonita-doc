...
=== Permissions
xref:ROOT:rest-api-authorization.adoc[REST API extensions, static and dynamic API security rules], xref:identity:bdm-access-control.adoc[BDM access control], xref:process:actor-filtering.adoc[actor filters], xref:process:actors.adoc[actor mapping], xref:profiles-overview.adoc[profiles] need to be specified and/or doulbe-checked at that time, to make sure that accesses are given to the right people for the right data.
