...
For Subscription editions users, this is also the time to configure xref:{bcdDocVersion}@bcd::index.adoc[Bonita Continuous Delivery]. +
 ...
 ** Pages developed with Svelte are embedded in the Development Suite Tomcat and can be found in the Resources page of the Bonita Administrator Application. Those pages cannot be edited in the UI Designer but they can be modified using an IDE (Integrated Development Environment) if needed.
...
Note: While the Bonita Applications are supported by Ofelia in their original versions, any customization of yours cannot be supported, as Ofelia cannot garantee the content of your custom development. +
