...
In the following example, we have a contract for a business object `Person`, with to contract inputs: a name and a birthdate. We also have a constraint on the birthdate: it cannot be in the futur. The test validate that if the birthdateContractInput is a date in the futur, then in instanciation of the process fails.
