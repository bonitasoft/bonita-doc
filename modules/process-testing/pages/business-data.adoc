...
<2> A relation field is retrieve from the generic business data: the method `getReference` is used to indicate that the field to retrieve is a reference (i.e a relation field), and the name of the field is passed to the method. Lazy and eager references are retrieved the same way.
...
<3> The relation field `singleRef` is retrieved using the Java method `getSingleRef()`. An object of type `ChildReference` is returned.
...
While the generic option doesn't require any development and is fully functional, the typed option requires the development of a Java model matching the business data model in order to manipulate strongly typed objects when using Business data.
...
A typed DAO works in the exact same way as a generic DAO, but the returned objects are strongly typed. +
...
<3> The `find` query is executed and a list of `MyObject` is returned.
