...
You can xref:live-update.adoc[update a form in production], by exporting a new version of the form from the Bonita UI designer, importing it into the relevant process in the portal, and then mapping it to the relevant task or process start event.
...
See xref:variables.adoc]#business-data[Data management] section to know how to use this Business Data Model.
