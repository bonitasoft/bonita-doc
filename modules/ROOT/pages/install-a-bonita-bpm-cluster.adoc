...
. Once you have done all the steps from section *Create and initialize the database for Bonita BPM Platform* above,  run `setup.sh configure` or `setup.bat configure` as described in the link:BonitaBPM_platform_setup.md#run_bundle_configure[Bundle configuration] to have your Tomcat / WildFly bundle configured to point to the right database.
