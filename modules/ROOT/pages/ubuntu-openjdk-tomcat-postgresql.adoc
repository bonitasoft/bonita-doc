...
To install the OpenJDK JVM you need to install the `openjdk-11-jre-headless` package:

* Run the following command line: `sudo apt install openjdk-11-jre-headless`
...
* `sudo tar xzvf apache-tomcat-8.5.66.tar.gz -C /usr/share/tomcat8 --strip-components=1`
 ...
 ** For `License type:`
  ...
  *** enter `2` to select `2 - CPU core license.` If your subscription type is cpu based, please refer to the https://customer.bonitasoft.com/knowledgebase[knowledge-base] in the Customer Service Center.
...
* Connect to Bonitasoft https://customer.bonitasoft.com/[Customer Service Center]
