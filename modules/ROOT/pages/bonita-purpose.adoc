...
Bonita Administrator Application is another application used by the tenant administrator to xref:admin-application-process-list.adoc[install, deploy and manage processes]. We use the word "tenant" here, in case the platform would be made of xref:multi-tenancy-and-tenant-configuration.adoc[several tenants].
