...
There are two cases: 
  * The flow node is contained in a parent flow node. This can happen in case of:
    ** A loop
    ** A multi-instantiated task
    ** Sub-tasks
    This is not the case for boundary events: they are at the "same level" as normal flow nodes, their parent being the process and not the flow node they are attached to.
   ...
   * The flow node is directly contained in a process instance. This is the nominal case. 
     . The childFinished() method (of the processExecutor) is called
     . The method executeValidOutgoingTransitionsAndUpdateTokens() is called on the process defintion
       ...
       * If it returns true, the flow node that finished was the last flow node of the process, and we terminate the process

=== Aborting/Cancelling a flow node
