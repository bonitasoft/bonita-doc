...
_Symptom_: After migrating to 7.11.0+, prior to 7.11.6, after re-installing/updating the BDM, some foreign key constants are effectively duplicated: there are foreign keys that refer to the same columns and tables, but with a different name.

_Cause_: A bug in an underlying product dependency. The upgrade of an internal library, from Hibernate 4 to 5, introduces a bug ( xref:https://hibernate.atlassian.net/browse/HHH-13779[HHH-13779]] )

_Solution_: Doing nothing is an option, as there is no change in product behaviour. If you wish to clean your BDM database, follow the following procedure:
...
This is fixed in 7.11.6, without any action required from the user.
