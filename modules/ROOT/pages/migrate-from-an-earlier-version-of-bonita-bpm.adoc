...
:description: This page explains how to update your Bonita Runtime to a newer version of Bonita.

{description}

== Overview

To update Bonita Runtime you need to use the _Bonita Migration Tool_. There are 2 versions of the migration tool:
...
For example, if you consider updating from 6.5.3 to 7.1.0, you must first update from 6.5.3 to 7.0.0 using version 1.x of the _Bonita Migration Tool_, and then update from 7.0.0 to 7.1.0 using version 2.x.
You are recommended not to start 7.0.0 after you update to it, but to proceed immediately with the second phase of the update.
...
When you are ready to update from 6.5.4 to a 7.x.y version, make sure you update to at least 7.1.1 to be sure that these fixes are present in your new version after the update.
====

*JRE requirements:*

* Bonita versions 7.0 to 7.4 only support JRE version 7. If you come from older versions of Bonita supporting JRE 6, you must also update your JRE to version 7.
* Version 7.5 only supports JRE version 8. If updating to version 7.5, please update your JRE to version 8.
* Version 2021.2 only supports JRE version 11. If updating to version 2021.2 / 7.13, please update your JRE to version 11 as well.
...
The Migration Tool migrates your runtime (_bonita_home_ folder (up to Bonita 7.2.x and the database), so you cannot xref:upgrade-from-community-to-a-subscription-edition.adoc[change edition] at the same time as updating.
...
Starting from version 7.3 there is no more _bonita home_ folder. This means that, if your installation does not have any custom change, then you do not need to configure the bundle any further for an installation updated to 7.3 or above.

On the other hand, if you have customized your configuration, you will have to use the xref:bonita-bpm-platform-setup.adoc#update_platform_conf[setup tool] to send your customized configuration files to the database where the configuration is stored, for versions 7.3 and above.
====
...
The _Bonita Migration Tool_ contains a set of script that execute changes on the data to make it compatible with the newer version of Bonita Runtime.
...
When the script has finished, you need to complete the update procedure by unzipping and configuring a bundle for the new version.
...
* (Before Bonita 2021.2:) There is no guarantee that the Look & Feel definition is compatible across maintenance versions.
...
Starting wiht Bonita 2021.2, there is no more Portal. Bonita is all applications. If you need to use some of the Portal Look&Feel assets in the themes of your applications, make sure you create backups of those files.
====
...
*Important:* +
The updating operations reset the Bonita configuration files to their default versions, for new settings to be applied (from the _$BONITA_HOME_ folder in versions prior to 7.3.0 or inside the database in versions 7.3.0 and greater). +
Therefore, you must xref:bonita-bpm-platform-setup.adoc#update_platform_conf[backup your configuration files] before lauching the updating procedure. +
...
Therefore, it is also strongly recommended to xref:back-up-bonita-bpm-platform.adoc[backup your database] before lauching the updating procedure.
====
...
target version, and the system configuration, so it is not possible to be precise about the time that will be required. However, the following example can be used as a guide:
...
== Updating Bonita in its Tomcat bundle
...
. If you use Oracle, there is already the driver for 19.3.0.0 oracle version in the `bonita-update/lib`. add the JDBC driver for your database to `bonita-update/lib`. This is the same driver as you have installed in your web server `lib` directory. You must upgrade to xref:migrate-from-an-earlier-version-of-bonita-bpm.adoc#oracle12[Oracle 12c (12.2.x.y)] in order to update Bonita to version 7.9 or greater.

WARNING: make sure you double check that you use the official driver version that matches your database version. The correct driver is mandatory for a smooth migration: xref:database-configuration.adoc#proprietary_jdbc_drivers[follow the instructions for Oracle driver download.]
...
. If you use Microsoft SQL Server, add the JDBC driver for your database type to `bonita-update/lib`. This is the same driver as you have installed in your web server `lib` directory.
...
Note: If you are using MySQL, add `?allowMultiQueries=true` to the URL. For example,
    ...
    Also, if you are updating to Bonita 7.9 or greater, you must update your database server to MySQL 8.0, see <<mysql8,Migrating to Bonita 7.9+ using MySQL>> specific procedure below.
====

. (Target version older than 2021.2:) If you used a custom Look & Feel in Bonita Portal, export it, and then restore the default Look & Feel.
  (Target version 2021.2 or greater:) Exporting the Look & Feel is not needed, as Bonita Portal has been replaced by Bonita applications. However, the backup of your Look & Feel files may be helpful to create the applications themes.
 ...
 ** If you are updating from a version older than Bonita 7.3.0. If you are using a BDM, your BDM will have to be redeployed after the update is done (see <<bdm_redeploy,above>>). In this case, it is recommended that you *stop your BPM services before updating*, so as to be able to redeploy your BDM immediately after the update (and not having to deal with eventual errors in automated processes as you restart your platform after the update procedure is over).
...
. *IMPORTANT:*
xref:back-up-bonita-bpm-platform.adoc[Back up your runtime nodes] and database in case of problems during the updating procedure.
 ...
 ** Starting from Migration Tool version 2.44.1, an additional script called `check-migration-dryrun` is present in the same folder. This script only run the checks the update would run, without actually migrating the elements. This is equivalent to running the migration script with a `--verify` option.
. The script detects the current version of Bonita, and displays a list of the versions that you can update to. Specify the
 version you require. Then it starts the updating procedure.
. As the script runs, it displays messages indicating progress. After each step, you are asked to confirm whether to
 ...
 property.
 When the migration script is finished, a message is displayed showing the new Runtime version, and the time taken for the migration of all elements. The database has been updated.

WARNING: Do not use the old Tomcat server: a new one needs to be installed with the Bonita binaries that match the target version.

. Unzip the target bundle version into a directory. In the steps below, this directory is called `bonita-target-version`
. xref:database-configuration.adoc[Configure the bundle to use the updated database]
 Do not recreate the database and use the setup tool of the `bonita-target-version` Edit the `bonita-target-version/setup/database.properties` file to point to the updated database.
. Reapply configuration made to the runtime, using the setup tool of the `bonita-target-version`
...
. If you have done specific configuration and customization in your server source version, re-do it by configuring the application server at `bonita-target-version/server` (or `bonita-target-version` if the target version is Bonita 7.3.n or greater): customization, libs etc.
. *(For source version is Bonita 7.4 or greater:)* +++<a id="compound-permission-migration">++++++</a>+++
...
. Configure License: +
You need to put a new license in the database: see xref:bonita-bpm-platform-setup.adoc#update_platform_conf[Runtime configuration] for further details.
 There is below a Linux example:
+
[source,bash]
----
...
. Start the application server. Before you start Bonita Portal (before 2021.2) and/or Bonita Applications, clear your browser cache. If you do not clear the cache, you might see old, cached versions of Portal or Applications pages instead of the new versions.
. Log in to Bonita UIs and verify that the updating procedure has completed.
. *If the target version is Bonita 7.7 or greater*
 If you used the _Migration Tool_ 2.41.1 or greater, the table `arch_contract_data` is automatically backed up to the table `arch_contract_data_backup` to avoid long lasting migration.
 To reintegrate the data into your installation, a new tool is provided in versions 2.46.0 and above. It is located in the `tools/live-migration` folder. Follow instruction in the README.md to run this tool and re-integrate data from `arch_contract_data_backup`.
. *If the target version is older than Bonita 2021.2*
If you did not set the default Portal Look & Feel before updating and you cannot log in, you need to restore the default Look & Feel using a REST client or the Engine API.

The update is now complete.
*If the target version is older than Bonita 2021.2"
If you were using a custom Look & Feel before the update, test it on the new version before applying it to your updated Runtime.

== Migrating processes with 6.x forms and case overview pages

Until the Bonita 7.0.0, Bonita used UI artifacts based on the Google Web Toolkit (GWT) technology: process instantiation forms, task execution forms and case overview page.
...
Specifically if you are updating from a Bonita 6.x version:
...
If you are migrating from a Bonita 7.x version:
...
* Disable the versions of your processes using 6.x forms. Make sure they have no more running instances.
* Perform the updating procedure to the desired version.

The disabled processes with 6.x forms cannot be enabled again post migration. +
...
It means that you might want to redo the case overview page as well as the forms, especially if you have configured a custom case overview page for your processes in version 6.x. +
(Enterprise, Performance, and Efficiency editions only) you can live update the case overview page after the update is complete.
...
(created with the UI Designer), when your sources version is older than Bonita 7.8.0. This allows you to see if the page suits your needs. If not, it can be used as a base to customize your case overview page. Your pages will then be ready for the 7.8.0 step of the Bonita Runtime update.

To run it, unzip the latest Migration Tool and run:
...
== Updating to Java 11 in Bonita 7.9 or greater version
...
* Update Bonita Runtime to Bonita 7.9.0 as usual, and keep running it in Java 8. Verify that everything works as expected.
...
== Updating to Bonita 7.9 or greater version using PostgreSQL
...
== Updating to Bonita 7.9 or greater version using MySQL

Bonita 7.9 and greater versions support MySQL 8.0.x version, which is not compatible with older versions of MySQL. For this reason, to update to Bonita 7.9 or greater version when using MySQL, follow this procedure:
...
== Updating to Bonita 7.9 or greater version using Oracle
...
If the target version is Bonita 7.8.4, skip this section and jump directly to the *Upgrade Oracle database server* section .

. Shut down Bonita Runtime
. Run the Migration Tool to update to Bonita 7.8.4, following the updating procedure <<migrate,above>>
...
=== Update to Bonita 7.9 or greater version

. Shut down Bonita Runtime
. Run the Migration Tool to update to Bonita 7.9 or greater version, following the migration procedure <<migrate,above>>
. Update your Oracle database server to the version 12c (it must be 12.2.x.y)
. Run the Migration Tool again to update Bonita to 7.9 or greater version
. Restart the new Bonita Runtime

[#oracle19]

== Updating to Bonita 7.11 or greater version using Oracle

Bonita 7.11 and greater versions support Oracle 19c version. To update to Bonita 7.11 or greater version when using Oracle,
...
To do so (for example going from 7.11.0 to 7.11.1), follow the following steps:
...
*NB* : In a cluster environment, you need to stop all your nodes and update them before starting them with the new maintenance version.

[#migrateCluster]

== Update a Bonita Runtime cluster
