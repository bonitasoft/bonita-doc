...
This page explains how to update your Bonita Runtime to a newer version of Bonita.

You can update your Bonita Runtime using the `Bonita Migration Tool`. There are 2 versions of the migration tool:

* The version 1.x that migrate the elements of Bonita Runtime from version 6.0.2 or later to any version up to 7.0.0
* the version 2.x that migrate the elements of Bonita Runtime from version 7.0.0 or later to any version up to the last one.

For example, if you are updating from 6.5.3 to 7.1.0, you must update from 6.5.3 to 7.0.0 using version 1.x of the _Bonita Migration Tool_, and then update from 7.0.0 to 7.1.0 using version 2.x.

[CAUTION]
====
You should always update to the latest version of a main version. Usually, maintenance version might contain bug fixes made after the release of subsequent main releases.
...
To update from 6.5.4 to 7.1.1 or later there are two phases: first you update to 7.0.0 then to the latest version.
...
* Versions 7.0 to 7.4 only support JRE version 7. If you come from older versions of Bonita supporting JRE 6, you must also update your JRE to version 7.
...
The tool migrates your runtime (_bonita_home_ folder and the database). You cannot xref:upgrade-from-community-to-a-subscription-edition.adoc[change edition] while updating.

* If you are running a Bonita Subscription Pack edition, you need a valid license for your target version.
* If you are updating to a new maintenance version and not changing the main version number (for example, you are updating from x.y.0 to x.y.1), your current license remains valid after the update.
...
On the other hand, if you have customized your configuration, you will have to use the xref:bonita-bpm-platform-setup.adoc#update_platform_conf[setup tool] to send your customized configuration files to the database where configuration is stored, for versions 7.3 and above.
====
...
The _Bonita Migration Tool_ contains a set of script that execute changes on the data to make it compatible with earlier version of Bonita Runtime.
...
you need to complete the update procedure by unzipping and configuring a bundle for the new version.
See  <<migrate,Migrate your platform>> for step-by-step instructions.

== Constraints

* If you have added custom indexes to certain tables in the Engine database, you must remove them before updating to a later version. If you do not remove these indexes, the update procedure will not complete.
...
If you are using a custom Look & Feel for Bonita Portal,  xref:managing-look-feel.adoc[export] it before updating.
Then after the update is complete, xref:managing-look-feel.adoc[export the default Look & Feel] from the new version,
...
Therefore, it is strongly recommended to do a  xref:back-up-bonita-bpm-platform.adoc[backup of your database] before starting the update procedure.
====
...
This section explains how to update a Bonita Runtime that uses the Bonita Tomcat bundle.
...
. Check that your current RDBMS version is compliant with the versions supported by the target version of Bonita (see <<rdbms_requirements,above>>)
...
WARNING: make sure you double check that you use the official driver version that matches your Database version. The correct driver is mandatory for a smooth migration:  xref:database-configuration.adoc#proprietary_jdbc_drivers[follow instructions for Oracle driver download.]
Particularly, if you use Oracle 12.2.0.x.y and are updating to 7.9.n or to 7.10.n, then remove the existing `ojdbc8-19.3.0.0.jar` file, and add the specific JDBC driver to `bonita-migration/lib`.
...
Note: If you are using MySQL, add `?allowMultiQueries=true` to the URL. For example,
    ...
    Also, if you are updating to Bonita 7.9+, you must update your database server to MySQL 8.0, see <<mysql8,Migrating to Bonita 7.9+ using MySQL>> specific procedure below.
====

. If you used a custom Look & Feel in Bonita Portal, xref:managing-look-feel.adoc[export] it, and then xref:managing-look-feel.adoc[restore the default Look & Feel]. This is not needed when updating to Bonita 2021.2 or later versions, as Bonita Portal has been replaced by Bonita applications. However, the backup of your Look & Feel files may be helpful to create the applications themes.
. Generally speaking, pausing (or not) the BPM services before the update should not impact the update procedure. There are however a few special cases:
 ** If you are updating from a version *< 7.3.0*. If you are using a BDM, your BDM will have to be redeployed after the update is done (see <<bdm_redeploy,above>>).
In this case, it is recommended you *stop your BPM services before updating*, so as to be able to redeploy your BDM immediately after the update (and not having to deal with eventual errors in automated processes as you restart your platform after the update procedure is over).
 ...
 ** Therefore, while it is recommended to always update to the latest version of Bonita, if your target version is not 2021.1, or above, it is necessary to update with *BPM services running*.
. Stop the application server.
. *IMPORTANT:*
xref:back-up-bonita-bpm-platform.adoc[Back up your platform] and database in case of problems during the update procedure.
 ...
 ** Starting from version 2.44.1, an additional script called `check-migration-dryrun` is present in the same folder. This script only run checks the update would without actually migrating the elements. This is equivalent to running the migration script with a `--verify` option.
. The script detects the current version of Bonita, and displays a list of the versions that you can update to. Specify the
 version you require.
 The script starts the updating procedure.
. As the script runs, it displays messages indicating progress. After each migration step, you are asked to confirm whether to
 ...
 property.
 When the migration script is finished, a message is displayed showing the new Runtime version, and the time taken for the migration of all elements.
...
. *If your Bonita version is 7.4 or above before the update, you can skip this point.* +++<a id="compound-permission-migration">++++++</a>+++
 ...
 ** Perform a diff between the version before update and the version after update of `tenants/[TENANT_ID]/conf/compound-permissions-mapping.properties` and put the additional lines into the file `tenants/[TENANT_ID]/conf/compound-permissions-mapping-custom.properties`
 ** Perform a diff between the version before update and the version after update of `tenants/[TENANT_ID]/conf/resources-permissions-mapping.properties` and put the additional lines into the file `tenants/[TENANT_ID]/conf/resources-permissions-mapping-custom.properties`
 ** Perform a diff between the version before update and the version after update of `tenants/[TENANT_ID]/conf/dynamic-permissions-checks.properties` and put the additional lines into the file `tenants/[TENANT_ID]/conf/dynamic-permissions-checks-custom.properties`
...
. Start the application server. Before you start Bonita Portal (and/or Bonita Applications), clear your browser cache. If you do not clear the cache, you might see old, cached versions of Portal or Applications pages instead of the new version.
 Log in to Bonita UIs and verify that the update procedure has completed.
 If you did not set the default Look & Feel before the update and you cannot log in, you need to  xref:managing-look-feel.adoc[restore the default Look & Feel] using a REST client or the Engine API. This is only true for target versions older than 2021.2.
. *If you have updated to a version 7.7 or greater*
...
The update is now complete. For target versions older than 202.2, if you were using a custom Look & Feel before the update, test it on the new version before applying it to your updated Runtime.
