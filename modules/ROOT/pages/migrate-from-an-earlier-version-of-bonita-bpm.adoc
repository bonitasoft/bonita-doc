...
For example, if you consider updating from 6.5.3 to 7.1.0, you must first update from 6.5.3 to 7.0.0 using version 1.x of Bonita Migration Tool, and then update from 7.0.0 to 7.1.0 using version 2.x. +
...
If you are using MySQL, add `?allowMultiQueries=true` to the URL. For example,
...
. *IMPORTANT:*
xref:back-up-bonita-bpm-platform.adoc[Back up your runtime nodes] and databases in case of problems during the updating procedure.
...
. Start the application server. Before you start Bonita Portal (before Bonita 2021.2) and/or Bonita Applications, clear your browser cache. If you do not clear the cache, you might see old, cached versions of Portal or Applications pages instead of the new versions.
...
*If the target version is older than Bonita 2021.2"
If you were using a custom Portal Look & Feel before the update, test it on the new version before applying it to your updated Runtime.
...
If you are updating from a Bonita 7.x version:
...
(Enterprise, Performance, and Efficiency editions only) you can xref:live-update.adoc[live update] the case overview page after the update is complete.
...
*Note*: In a cluster environment, you need to stop all your nodes and update them before starting them with the new maintenance version.
