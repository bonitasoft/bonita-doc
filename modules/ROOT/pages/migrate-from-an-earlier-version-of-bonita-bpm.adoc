...
. +++<a id="compound-permission-migration">++++++</a>+++ In the case where deployed resources have required dedicated xref:resource-management.adoc]#permissions[authorizations to use the REST API], these authorizations are not automatically migrated.
+
Some manual operation have to be done on files that are  located in the _$BONITA_HOME_ folder if version <7.3.0 or in the extracted `platform_conf/current` folder in version >=7.3.0 (see xref:BonitaBPM_platform_setup.adoc]#update_platform_conf[Update Bonita BPM Platform configuration] for more information ). You need to merge the previous file version and the migrated one.
