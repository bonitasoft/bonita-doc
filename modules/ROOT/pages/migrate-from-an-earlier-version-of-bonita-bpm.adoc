...
_Symptom_: After migrating to 7.11.0+, prior to 7.11.6 and latest 2021.1, after re-installing/updating the BDM, some foreign key constants are effectively duplicated.

_Cause_: A bug in an underlying product dependency (hibernate: )The upgrade of an internal library, from Hibernate 4 to 5, introduces a bug ()

_Solution_: Doing nothing is an option, as there is no change in product behaviour. If you wish to clean your BDM database, follow the procedure:


1. Stop your bonita server

1. Open the db in an edition tool (or execute in command line the relevant commands):
First, select all the foreign key on a table:
...
1. Repeat for all the tables of your BDM database.

1. Start your bonita server

::: Note : Guidance is not provided for Oracle, as the situation is impossible. Instead, the BDM redeployment will fail after migration.
This is fixed in the last 2021.1, without any action required from the user.
