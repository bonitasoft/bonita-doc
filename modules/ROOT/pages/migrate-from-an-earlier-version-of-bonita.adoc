...
:page-aliases: migrate-from-an-earlier-version-of-bonita-bpm.adoc
...
* Version 2021.2 only supports JRE version 11. If migrating to version 2021.1 / 7.13, please upgrade your JRE to version 11.
...
On the other hand, if you have customized your configuration, you will have to use the xref:bonita-platform-setup.adoc#update_platform_conf[platform setup tool] to send your customized configuration files to the database where configuration is stored, for versions 7.3 and above.
====
...
* Configuration of the platform: Before 7.3 in the _Bonita Home_ folder and after 7.3 in database. Reapply your customizations manually after the migration script has finished (using xref:bonita-platform-setup.adoc#update_platform_conf[platform setup tool] if migrated to 7.3.0+).
...
Therefore, you must do a  xref:bonita-platform-setup.adoc#update_platform_conf[backup of your configuration files] before starting the migration. +
...
+
Please refer to the guide on updating the configuration file using the  xref:bonita-platform-setup.adoc#update_platform_conf[platform setup tool]
+
...
. *If your Bonita version is 7.4 or above before migrating, you can skip this point.* +++<a id="compound-permission-migration">++++++</a>+++
 ...
 Some manual operations have to be done on files that are  located in the extracted `platform_conf/current` folder (see  xref:bonita-platform-setup.adoc#update_platform_conf[Update Bonita Platform configuration] for more information). You need to:
...
. Configure License:
+
you need to put a new license in the database: see  xref:bonita-platform-setup.adoc#update_platform_conf[Platform configuration] for further details.
Below a Linux example:
+
[source,bash]
----
 ...
 ** xref:bonita-platform-setup.adoc#update_platform_conf[Platform configuration]
...
. *If you migrated past version 7.7*
...
The 7.9.0 migration step tries its best to migrate the implementation of connectors that are known *not to* work in Java 11, namely WebService, CMIS, Email and Twitter. +
...
Thus, thorough tests have to be carried out so ensure there is no regression when migrating Bonita to version 7.9+.
...
* check you can successfully log into the Bonita Runtime
