...
It is now possible to retrieve also the tasks assigned to other users when getting the assigned or pending tasks for a given user by combining the filters `user_id` and `show_assigned_to_others` of xref:api:activities-tasks-flow-nodes-api.adoc#_search_for_a_humantask[`bpm/humanTask` REST API resource].
