...
xref:identity:rest-api-authorization.adoc#dynamic_authorization[Dynamic authorization checking] is now only available in the *Enterprise* editions, and is now *active by default* for new installations. +
...
Community users that update to 2022.1 simply lose the possibility to use dynamic authorization checking.
Their potential previous configuration regarding dynamic authorizations is kept unchanged during the update procedure,
to be used as a reference for custom usage, or ready to use for a potential upgrade to Enterprise edition.
