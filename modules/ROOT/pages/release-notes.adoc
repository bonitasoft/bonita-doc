...
From version 2022.1-u2, if you embed *bonita-server(-sp).jar* in your application, you need to declare some new Maven Repository in you pom.xml / build.gradle. This is due to some Security Fix that requires a dependency that is NOT present in default Maven Repository (aka Maven Central). To do so, add the following code:
