...
Several REST resources were added for Task Delegation:
...
* {openApiUrl}/{openApiLatestVersion}/#tag/BPM[GET /API/bpm/processName]: read-only; searches process deployment information grouped by name and display name, returning one entry per distinct name with its deployed versions. Available to any client.
* {openApiUrl}/{openApiLatestVersion}/#tag/Identity[GET /API/identity/userSummary]: read-only; returns a lightweight, paginated list of users (id, user name, first name, last name, and job title) for user pickers, without the full `identity/user` payload. Available to any client.
