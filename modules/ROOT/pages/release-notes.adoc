...
The Studio is stripped of its major coding editor (except for the REST API Extensions), and has now a xref:bonita-overview:managing-extension-studio.adoc[dedicated view for extensions]. This view is used to get extensions from different sources:
...
* STUDIO-4188 - Switch to a bonita project after renaming it
...
* RUNTIME-586 - CORS requests enabled to work with OpenID Connect for preflight requests
* RUNTIME-563 - The case comments in the task details disappears after a task is completed
* RUNTIME-542 - Have the possibility to add a comment when you execute a task with no form on someone else's behalf
* RUNTIME-541 - Process instances started on someones behalf displayed as such in the case Details page
* RUNTIME-537 - Execute the task on behalf of someone from new Admin Task Details sets the executedBySubstitute
* RUNTIME-527 - Case details and Task details does not display 'displayDescription'
* RUNTIME-518 - REST API caseVariable returns 500 error message when p greater than number results in database
* RUNTIME-473 - generateRequestKey script and Bonita Tomcat bound to use the very same 'java' binary
* RUNTIME-472 - Redirection to login page after logout of Super Admin user
