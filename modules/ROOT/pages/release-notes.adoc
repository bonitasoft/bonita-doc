...
==== 4xx errors handling
When there is a 4xx error while trying to display a resource like 404 (not found) of 403 (forbidden), the status code is set in the HTTP response header, but Bonita applications no longer try to display the dedicated error pages unless the resource is a page. So the status code remain 4xx and is not changed to 200 with the error page in the body of the reponse.
