...
`TenantAdministrationAPI` `pause`, `resume`, and `isPaused` Java API methods and `system/tenant` REST API are now deprecated. Use java `MaintenanceAPI` and REST `system/maintenance` APIs instead, to activate/deactivate the maintenance mode and get the maintenance state.
