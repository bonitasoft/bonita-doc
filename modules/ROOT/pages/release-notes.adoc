...
[NOTE]
====
The 2022.2 is in qualification.
====
...
Bonitasoft has launched the open source project Process Analytics two years ago, as a response to market's need in bpmn process monitoring execution using bpmn standards. +
...
The new values are available for Bonita Subscription Editions starting with the Efficiency.
  ...
  * Since the pages have been created by using Bonita UI Designer, developers can:
        ...
        ** Offer process execution data visualization to specific users by adjusting the permissions accordingly
