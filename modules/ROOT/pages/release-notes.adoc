...
Welcome to our xref:bonita-applications-interface-overview.adoc[*four Bonita applications*], that beautifully replace our good old Bonita Portal, gone for retirement. +
You will find more information in <<New Bonita Applications>>. +
To know more about Bonita Applications, go to xref:bonita-applications-interface-overview.adoc[this overview page].
Custom Profiles with a Portal navigation will be automatically convertedto _custom applications_ upon Bonita update. Be extra-careful during the Bonita update procedure.
====
...
Automation projects now use the power Maven projects. You are now able to easily compose your project of extensions (connectors, Rest API extensions, themes, and actor filters). +
...
As connectors are now extensions of a project, their version is now bound to Bonita Studio version, therefore you do not need to update Bonita Studio to migrate a connector or to migrate your connector when updating Bonita Studio. This will simplify version updates. +
...
It is now possible to xref:single-sign-on-with-oidc.adoc[configure Bonita to use OpenID Connect (OIDC)] as an authentication mechanism to access your Bonita applications and achieve Single Sign-on with an external OIDC provider. +
...
In Bonita 2021.2, custom profile navigation has been removed, and thus profiles with navigation are automatically converted into custom applications by the xref:migrate-from-an-earlier-version-of-bonita.adoc[migration tool].
...
.The previous Custom Profile navigation is now accessible as a custom application
...
In the Community edition, if you are updating from Bonita 2021.1 or an earlier version, you will need to xref:bonita-bpm-platform-setup.adoc[change your configuration] file `authenticationManager-config.properties` and remove the JAAS configuration to use another Authentication Manager (an implementation of your own, for instance).
