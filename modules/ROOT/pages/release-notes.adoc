...
* In Bonita Tomcat bundle, HTTP APIs are *active by default*, and *default* credentials of xref:api:configure-client-of-bonita-bpm-engine.adoc#configure-client-using-programmatic-configuration[client configuration using programmatic configuration]. They can be changed in file `<BONITA>/server/conf/tomcat-users.xml`. 
