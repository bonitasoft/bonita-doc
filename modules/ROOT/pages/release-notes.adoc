...
As the extensions are developed outside the Studio, they are no longer bound to a specific Bonita project. They can be easily re-use in a different project - thus improving your efficiency.

The Studio is stripped of its major coding editor (except for the REST API Extensions) but has now a dedicated view for extensions. This view is used to get extensions from a different source: 
...
As connectors are now extensions of a project, their version is now bound to the Studio version, therefore you don’t need to upgrade your Studio to upgrade a connector or to upgrade your connector when upgrading your Studio.

This will simplify version upgrades.

Finally, your sources will be lighter as the dependencies will be pulled by the project.

== Organization Editor

Organization editor has been reworked, to improve your experience and your efficiency.
...
2021.1 introduced a new expression editor. This new expression editor covered most of the existing editor type use case.
...
If your project are using these expression types, they will migrated to the new expression editor when you import your project in the 2021.2 Studio.
