...
A health check endpoint has been added to Bonita Runtime for helping you monitor the status of all engine layers, up to the database: 
...
Credentials can be consulted and modified:

* in the <BONITA>/server/conf/tomcat-users.xml file in a *Bonita Tomcat bundle*
* using xref:bonita-docker-installation.adoc#_monitoring_username[monitoring username and password] when starting a *Bonita Docker image*
...
** username: `http-api`
** No default password. One must be provided, otherwise the Docker container will refuse to start.
...
Dynamic authorization checking is now *active by default* for new *Enterprise* edition installations. +
When updating to 2022.1 from a previous Bonita version, the existing activated permission mechanism will not be changed. 
...
NOTE: We provide pre-configured database images with included schema and users on https://hub.docker.com/u/bonitasoft[Docker Hub].
...
To make the Bonita Docker image configuration easier, new environment variables have been added:
