...
xref:bonita-applications-interface-overview.adoc[Those new applications] beautifully replace our good old Bonita Portal, gone for retirement. +
...
The Studio is stripped of its major coding editor (except for the REST API Extensions), and has now a xref:managing-extension-studio.adoc[dedicated view for extensions]. This view is used to get extensions from different sources: 

* From the Bonita marketplace, which contains all the connectors and actor filters made and maintain by Bonitasoft
