...
Each development team using the Bonita Platform has several options.
Here are the options proposed by Bonitasoft:

. Create a dedicated project that is completely independent of the existing Bonita projects (i.e. the generated application will be deployed independently of the existing ones)
...
For those who want to try out deploying a Bonita project as a self-contained application, we have developed an *experimental* packaging tool. The outcome will be an application packaged either in Tomcat Bundle or a docker image, you choose.
...
[WARNING]
====
The update process of the self-contained application packaged with the *experimental* tool will not be covered with xref:version-update:update-with-update-tool.adoc[Update Tool], hence we advice deployment in non-production environments.
====
...
Bonita 2023.1 introduces a new multi-runtime architecture. This change comes with a new way to handle multi-tenant use cases and thus the removal of the multi-tenancy as it is currently known.
...
* possibility to integrate additional information from the Identity Provider of the SSO at user account creation (like first name, last name, email, etc...)
* user groups can be retrieved from the SAML response or OIDC ID token and created automatically in a Bonita Runtime database at user account creation
...
xref:data:define-and-deploy-the-bdm.adoc#build-and-deploy-bdm-artifacts[Business Data Model content check at build or deploy] will still be the developer’s responsibility along with handling data-deletion risks at build or deploy.
...
* The default widgets in the palette that contain an HTML input tag now have an associated label, which will facilitate the use of computer tools (screen readers, for example)
...
==== Security fixes for all the application pages developed using UI Designer
...
The first step of the long-term project we initiated is correcting all critical and high CVEs in the AngularJS version used to run our Bonita application pages developed using UI Designer.
...
When updating a Bonita project in a Bonita Studio 2023.1 version, BDM dependencies will be directed to use the new the `*-bdm-model` artifact introduced with the Bonita project layout changes. Meaning that `bdm-client` and `bdm-dao` dependencies will automatically be replaced with project's `bdm-model` dependency.

Additionally, the dependency containing the client DAO implementation will be removed from the dependency list. It can affect the code directly referencing the `*DAOImpl` classes. To avoid this situation, change the code to use the DAO interfaces present in the `bdm-model` dependency. The changes can be done before or after updating to the 2023.1 version without breaking the project.
...
For customers running a Bonita multi-tenancy platform, the technical path from multi-tenancy proposed by Bonitasoft is multi-runtime. This choice will require splitting the current multi-tenant platform into several runtimes, each one with their own Bonita engine database.

As this path can be challenging for customers using multi-tenancy, Bonitasoft developed a conversion tool, available for download {cscCustomerServices}[Customer Service Center]. We strongly suggest that our customers take the time and use the tool in a pre-production environment before using it on a production environment.
...
=== Fixes in Bonita 2023.1-u6 (2024-09-09)
