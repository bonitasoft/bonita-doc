...
=== AngularJS fix of version used by Bonita UI Designer Artifact builder

In order to fix CVEs in Bonita UI Designer Artifact builder, we have made a fork of the AngularJS project and cherry-picked the fixes to the version that is used by the module.

More information about this can be found in xref:version-update:angular-js-UID-fork.adoc page.

=== Default application AngularJS pages version update

An update to the latest version of AngularJS (1.8.3) was done on some default pages that are provided by Bonita. +
The pages that are concerned by this change are:
