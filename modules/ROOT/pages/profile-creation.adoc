...
* *Save* profiles file *As* another file of profiles (useful to duplicate the whole file)
* *Deploy* profiles file (i.e all the profiles in this file) +
⚠  For _development_ purposes, it also deploys their referenced pages used as portal menu entries, but if those resources use REST API Extensions, you have to deploy those manually. In _production_ environment, you still need to import pages, and REST API Extensions manually *before* importing profiles file.
...
Pages proposed are those provided by default in the Bonita Runtime and those created with the UI Designer. You can reference a custom page only if it is imported in the Bonita Runtime, and you will have to write its exact identifier.
