...
See the xref:identity:organization-overview.adoc[Organization overview] for details.
...
See xref:ROOT:organization-management-in-bonita-bpm-studio.adoc[Configure a test organization] and xref:process:actors.adoc[actors] for details.

== Environments and parameters

An environment (available in Bonita Subscription editions only) is the context in which a process will run. +
