...
Bonita BPM uses http://www.keycloak.org/[Keycloak] as SAML service provider adapter.
====
...
When a user logs out from the IdP, Bonita Portal's session will remain active. The user's session time to live will be reset
...
The recommended way to authenticate to Bonita BPM Portal to use the REST API is to use the xref:rest-api-overview.adoc]#bonita-authentication[login service]..
