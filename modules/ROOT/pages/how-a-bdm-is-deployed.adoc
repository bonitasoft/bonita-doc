...
Deploying a BDM means calling https://javadoc.bonitasoft.com/api/{javadocVersion}/org/bonitasoft/engine/api/TenantAdministrationAPI.html#installBusinessDataModel-byte:A-[TenantAdministrationAPI.installBusinessDataModel()].

As an input parameter, we pass a *ZIP file* containing an XML file, like the one below:
...
Regarding the *deployment of a new version of the BDM*, `Hibernate hbm2ddl` can handle several changes, but not all
...
However, some refactoring just do nothing to the database schema, like changing a column type, removing an attribute or an object...
