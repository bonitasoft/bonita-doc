...
All nodes must run the same exact version of the Bonita Platform. e.g. a `7.11.2` will not be able to connect to a cluster of Bonita Platform in `7.11.1`. 
...
=== Upgrading the version of a Bonita Platform in cluster
...
Since all nodes must use the exact same version of the Bonita Platform, the cluster must be shut down entirely to be able to change the version.
...
When migration to a different release of the Bonita Platform, the database also need to be migrated. See xref:migrate-from-an-earlier-version-of-bonita-bpm.adoc#migrateCluster[here]