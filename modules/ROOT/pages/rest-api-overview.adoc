= REST API overview
:description: This page contains an overview of how to integrate an application with Bonita BPM using REST.
...
. xref:bpm-api.adoc]#human-task[List the pending tasks for a user]: Retrieve a list of available human tasks for the logged in user. When the user selects a task to do, you can display the corresponding form. It can be an external form or a Bonita BPM form that can be accessed by url.
