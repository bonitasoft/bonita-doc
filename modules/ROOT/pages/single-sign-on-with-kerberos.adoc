...
* The Bonita Kerberos filter will automatically create a Bonita session and let the user through to access the Bonita Application resources.
...
The most commonly used solution is to hide the logout button from the Bonita Layout. Users are logged in as long as they don't close their web browser (unless their session times out). +
