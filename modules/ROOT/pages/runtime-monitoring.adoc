...
The runtime uses https://micrometer.io/[Micrometer] to produce and publish *metrics*. *Metrics* are indicator (generally a numeric value)
...
It is a channel on which metrics are published. Provided metric publishers are JMX, Log files, Prometheus.

=== JMX Publisher

The JMX publisher publishes each metrics using MBeans. All those MBeans are in the domain `metrics`.

It can be enable using the property `org.bonitasoft.engine.plugin.monitoring.publisher.jmx.enable` set to `true`.
...
It can be enabled using the property `org.bonitasoft.engine.plugin.monitoring.publisher.logging.enable` set to `true`.
