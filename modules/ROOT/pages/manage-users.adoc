...
In such case, there is another http://documentation.bonitasoft.com/javadoc/api/{javadocVersion}/org/bonitasoft/engine/api/ProcessRuntimeAPI.html#updateActorsOfUserTask(long)[method] that can be used as a separate call to force the re-execution of the actor filter and thus update the list of possible users.
