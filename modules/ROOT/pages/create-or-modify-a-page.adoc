...
:description: Guidance to create and modify User Interfaces elements to embed in your processes / reference in your applications with Bonita UI Designer.

Guidance to create and modify User Interfaces elements to embed in your processes / reference in your applications with Bonita UI Designer.

== User Interfaces development overview

Your automation project can use xref:pages-develoment.adoc[pages] to display information and provide interaction controls to the end-users or administrators. +
It most of the times uses xref:forms-development.adoc[forms] to display and collect new or edited information. +
...
To xref:customize-living-application-theme.adoc[create a custom theme], developers needs to use their favorite develoment environment and xref:software-extensibility.adoc[make it available in the project]. +
...
Before creating a form, the xref:contracts-and-contexts.adoc[contract] must have been defined in its _Execution / Contract_ property tab, for each form at a different level: 

* An instantiation form must be created from the pool _Execution / Instatiation form_ tab
* A task form must be created from the human task _Execution / form_ tab
...
Custom widget is a special element that we will explain in a later chapter.
...
WARNING: an element that has not been designed with the UI Designer cannot be imported into the UI Designer.
...
Applications and its elements must be deployed onto the target environment, manually from the Super Administrator or Administrator applications, or automatically  When you deploy the process in Bonita using the `.bar` file, forms are imported and deployed. Pages and layout must be imported one by one.
