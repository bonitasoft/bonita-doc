...
. Add the CAS module. To do this, copy cas-3.3.1-jbossas7-module/org to JBOSS_HOME/modules to merge the CAS module with the existing modules.
. Make the CAS module global so that it can be used by any application. To do this, edit `JBOSS_HOME/standalone/configuration/standalone.xml` and change the definition of the `ee` subsystem to the following:
...
. Edit `JBOSS_HOME/standalone/configuration/standalone.xml` and add the BonitaAuthentication module.
...
Set the Java system property `java.security.auth.login.config` in the Tomcat startup script to point to the JAAS configuration file, xref:BonitaBPM_platform_setup.adoc[`jaas-standard.cfg`].
+
...
Bonita BPM uses the CAS LoginModule in the JASIG implementation, so see the CAS LoginModule section of the https://wiki.jasig.org/display/CASC/JAAS+Integration[Jasig documentation] for more information.
. Copy `cas-client-core-3.3.1.jar` from `BonitaBPMSubscription-7.3.x-deploy` into the `/lib` directory.
. Copy `commons-logging-1.1.1.jar` from `BonitaBPMSubscription-7.3.x-deploy` into the `/lib` directory.
...
. Copy `commons-logging-1.1.1.jar` from the `BonitaBPMSubscription-7.3.x-deploy` into `tomcat/lib`.
. Remove the `WEB-INF/lib/commons-logging-1.1.1.jar` file from the `tomcat/webapps/bonita.war`.
. Remove the `tomcat/webapps/bonita/WEB-INF/lib/commons-logging-1.1.1.jar` file (if it is present).
