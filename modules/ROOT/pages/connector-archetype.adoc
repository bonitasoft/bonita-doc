 ...
 ** A Bonita connector project depends on _org.bonitasoft.engine:bonita-common_. To avoid potential conflicts / errors at runtime, you should use the technical id of the Bonita version of your runtime environment.
...
The built archive can be found in here `target/[artifact id]-[artifact version].jar` after the build.
...
[NOTE]
====
This step is an example of how to publish a Bonita extension on a private maven repository (here Github packages). You can publish your extensions on any kind of maven repository.
====
