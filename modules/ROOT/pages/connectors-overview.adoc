...
A connector is an element in a process that accesses an external system to read or write information. If the xref:connectivity-overview.adoc[default connectors in Bonita] are not suitable, you can implement your own connector.
In the Bonita Performance, Efficiency, and Teamwork editions, you are recommended to use the xref:connector-development-toolkit.adoc[connector development toolkit] to create new connectors.
