...
* With lazy loading, the parent object instance is loaded. To load the child object instances, additional REST API calls are made. This is the option to choose when:

  ** the object is not edited very often in the cases lifetime
  ** the parent object along with all its relationships can get big, vertically and/or horizontally
  
* With eager loading, the parent object instance and the child objects instances are loaded at the same time. This is the option to choose when:

  ** the object can be edited quite often in the cases lifetime
