...
* With lazy loading, the parent object instance is loaded, and child object instances are loaded only when they are needed, by the means of additional REST API calls. This is the option to choose when the parent object along with all its relationships can get big, vertically and/or horizontally +
* With eager loading, the child object instances are loaded when the parent object instance is loaded. It is easier to use because no additional REST API calls are needed. This is the option to choose when the parent object along with all its relationships are neither deep nor large.  +

The default is lazy loading, the option recommended in Bonita. You may override this for any business object by configuring it to use eager loading when you specify
the object relationship properties. +

[WARNING]
====
If you are not sure about the final size of the parent object, we strongly recommend to use the default *lazy* loading option. Indeed, the eager loading option can have a huge impact on the platform performances, due to the additional data load and maintenance of the information in memory. +
If you decide to use the eager mode, we recommend to test this option with a test dataset that is the size of your production's expected dataset. 
====
