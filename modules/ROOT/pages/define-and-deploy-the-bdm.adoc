...
The <<bdm_spec,BDM specification>> explains the details of what you must configure in a BDM. Use Bonita BPM Studio to <<define_bdm,define a BDM>>.
...
When you are ready to go to production, you need to xref:database-configuration-for-business-data.adoc[configure a database and business data datasources for business data] in your production environment.
...
. In Bonita BPM Studio, open the *Development menu* in the cool bar.
...
. Click *Pause*.
. When the service is paused, go to the *Resources* menu and choose *Business Data Model*.
. Specify the file containing the business data model that you exported from Bonita BPM Studio.
...
When you design a process, you specify the variables to be used in the process. These can include variables that are instances of business objects defined in the BDM. You xref:specify-data-in-a-process-definition.adoc[define a business object variable] in Bonita BPM Studio in a similar way to defining other process variables.
