...
*Warning*: With eager loading, there is a high overhead for the additional data load and maintenance of the information in memory. However there is a saving in data access time because the data is already loaded.

[WARNING]
====
Bonita strongly recommends to use the default *lazy* loading option. The eager loading option has a huge impact on the platform performances. You should only use it if you really need all the child object instances to be loaded when the parent object instance is loaded. If you decide to use the eager mode, we recommend to test this option with a test dataset that is the size of your production. 
====
