...
* With lazy loading, the parent object instance is loaded. To load the children objects instances, additional REST API calls are made. This is the option to choose when:
  ...
  ** the parent object along with all its children can get big, vertically and/or horizontally
...
* With eager loading, the parent object instance and the children objects instances are loaded at the same time. This is the option to choose when:
  ...
  ** the parent object along with all its children are neither very deep nor large
...
[WARNING]
====
If you are not sure about the final size of the parent object with children, we do strongly recommend to use the default *lazy* loading option. Indeed, the *eager* loading option can have a huge impact on the platform performances, due to the additional data load and maintenance of the information in memory. +
Based on your analysis if you decide to use the eager mode, we recommend to test it with a test dataset that is the size of your production's expected dataset.
====
...
[IMPORTANT]
====
It is recommended to xref:back-up-bonita-bpm-platform.adoc[Back up the platform] and database before deploying a new BDM in case of problems during the update.
====
...
If overwriting is not the option for you, click on "Cancel", solve the conflicts by comparing the two BDMs in xml editors, make sure all processes and UIs comply with those changes, and import again."
...
In the case where a business object has to be initialized from task level inputs, you can set contract inputs from the *Add from data...* action like in <<initBDM,previous section>> and generate automatically operations by selecting *Auto-generate the initialization script*. The operations will be generated for business data attributes you selected. You can manage it inside the *Details* panel, *Execution* tab, *Operations* pane.
...
For that, you can use the alternative we provide, by uncommenting the following lines in file `platform_conf/current/tenants/[TENANT_ID]/tenant_engine//bonita-tenants-custom.xml` using xref:bonita-bpm-platform-setup.adoc#update_platform_conf[setup tool].
