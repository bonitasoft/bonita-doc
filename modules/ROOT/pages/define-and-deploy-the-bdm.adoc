...
[WARNING]
====
If you are not sure about the final size of the parent object with children, and if editing the children objects instances will not be very frequent, we do strongly recommend to use the default *lazy* loading option. Indeed, the *eager* loading option can have a huge impact on the platform performances, due to the additional data load and maintenance of the information in memory. +
