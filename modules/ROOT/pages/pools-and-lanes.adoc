= Pools and lanes
:description: A pool is a container for a process in a diagram. +
...
* Parameters (Performance, Efficiency, and Teamwork editions): xref:parameters.adoc[Define the parameters] of the lane. A parameter is like a variable but has a value that is fixed for a deployment of a process.
