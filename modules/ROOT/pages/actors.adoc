...
. Define the actors for the pool:
 ...
 .. Click *_Add_*. A new row is added to the table, with a temporary name for the new actor (for example, Actor1). To change the name, select it and type the name you want to use. You can also add a description.
  ...
  ... Select the actor that will be the initiator of the process cases, and click *_Set as initiator_*. If you do not define an initiator for the pool, the process cannot be started in the Bonita Applications. Instead, it will have to be started programmatically.
...
See xref:custom-user-information-in-bonita-bpm-applications.adoc[Custom User Information in Bonita Apps]
...
[NOTE]
====
To use actor filters in your processes, you first need to import the corresponding extensions from the xref:managing_extension_studio.adoc[extension view].
====
...
For information about creating a new actor filter, see xref:actor-filter-archetype.adoc[Creating an actor filter].
...
xref:organization-management-in-bonita-bpm-studio.adoc[define your organization in Bonita Studio] and upload it to Bonita Applications.
