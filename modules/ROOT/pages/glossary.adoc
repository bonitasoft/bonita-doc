...
Delegate::
The person who, during a delegation period, sees and executes the delegator's assigned tasks on their behalf.

Delegation::
A mechanism that lets a delegate see and execute an absent user's assigned tasks, for a defined period and a chosen list of processes, without reassigning them. Available in Subscription editions only. See xref:runtime:user-delegation.adoc[Delegations].

Delegator::
The absent user whose assigned tasks are covered by a delegate during a delegation period.
