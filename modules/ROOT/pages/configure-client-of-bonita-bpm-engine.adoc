...
By default xref:tomcat-bundle.adoc[Tomcat] and xref:jboss-bundle.adoc[JBoss] bundles along with the `bonita.war` from the xref:deploy-bundle.adoc[deploy bundle] are configured to accept connection for HTTP connections.
...
By default the xref:jboss-bundle.adoc[JBoss bundle] is configured to accept connection for EJB3.
...
So open the JBoss administration console and https://docs.jboss.org/author/display/AS71/Admin+Guide#AdminGuide-adduser.sh[create a user account] to be used for remote connections (for example, username _bonita_ and password _bpm_).
