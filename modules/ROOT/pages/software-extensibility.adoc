...
:description: Bonita BPM provides a rich set of features by default. These features are designed to meet the needs of most of your projects.
...
Bonita BPM comes with more than 80 xref:connectivity-overview.adoc[standard connectors] to the major information system components: major databases (Oracle, Microsoft, Postgres, etc.), SOAP Webservice, Salesforce, Email, etc.
...
This intended to be used for xref:automating-builds.adoc[automating process builds] in a continuous integration and testing environment.
