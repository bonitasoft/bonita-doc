...
. Create the database user(s) to be used by the Bonita Engine and grant the required rights:
+
SQL query >
+
[source,sql]
...
+
WARNING: Both the `db.user` and the `bdm.db.user` defined in the `<TOMCAT_HOME>/setup/database.properties` will require the following privileges, so the queries must be executed for both of them.
+
SQL query >
+
[source,sql]
...
. For the business database user (the one defined in `bdm.db.user`) grant this additional privilege:
+
SQL query >
+
[source,sql]
----
GRANT create sequence TO bonita;
----
