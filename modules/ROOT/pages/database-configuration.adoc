...
WARNING: Please note this procedure cannot be performed using the Bonita Studio. The Bonita Studio can run only on the H2 Database.  
...
NOTE: CREATE TABLE and CREATE INDEX privileges are not required after first start in normal use. +
...
WARNING: *Reminder:* There is a known issue between Bitronix (the Transaction Manager shipped by Bonitasoft in the Tomcat bundle and in the Tomcat directories of the Deploy bundle) and the Microsoft SQL Server driver
