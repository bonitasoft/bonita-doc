...
CREATE TABLE and CREATE INDEX privileges are not required for the *Bonita Engine database* RDBMS user after first start in normal use (however, they will still be required by the *business data database* user even beyond the first deployment of a BDM). +
If the same RDBMS user is used with the xref:migrate-from-an-earlier-version-of-bonita-bpm.adoc[migration tool], then this user needs such grants.
====
