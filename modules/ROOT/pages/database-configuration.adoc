...
If you use different users for the engine data database and for the business data database, then you must grant this privileges to both users. 

Also, note that the owner of the new schemas (engine data *and* business data) must own the following privileges:
...
The following queries must be done for the bonita engine *and* the business data users. For example, if the users for the Bonita BPM and BDM databases are `bonita` and `business_data` respectively, you need to run the queries for both users. 
+
SQL query >
+
[source,sql]
