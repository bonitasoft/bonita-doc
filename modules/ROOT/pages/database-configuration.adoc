...
Download the driver compatible with Oracle 12.2.0.1: https://www.oracle.com/database/technologies/jdbc-ucp-122-downloads.html[ojdbc8.jar] ( make sure it is the offical driver by checking the SHA1 Checksum: https://github.com/bonitasoft/bonita-doc/commit/60f439fd01536508df32658d0a416c49ac6f07fb[`60f439f`] )
...
WARNING: If you use different users for the engine database and for the business data database, then you must *grant the following privileges to both users*. 

To create each new database, you need a RDBMS user account that has sufficient privileges (i.e. privileges to create new schema).

Next, the owner of each new schema (i.e. engine data *and* business data) must own the following privileges:
...
CREATE TABLE and CREATE INDEX privileges are not required for the *engine database* user after first start in normal use (however, they will still be required by the *business data database* user even beyond the first deployment of a BDM). +
...
+
WARNING: The queries must be done for *both* the bonita engine *and* the business data database users. +
For example, if the users for the Bonita BPM and BDM databases are `bonita` and `business_data` respectively (the actual values correspond to the ones set in the properties `db.user` and the `bdm.db.user` defined in the `<TOMCAT_HOME>/setup/database.properties`), you need to run the queries for both users. 
+
*For the bonita engine database user (e.g. _bonita_):*
+
SQL query >
+
[source,sql]
...
----
+
*For the business data database user (e.g. _business_data_):*
+
Note that the business data database user (the one defined in `bdm.db.user`) requires one additional privilege: CREATE SEQUENCE
+
SQL query >
+
[source,sql]
