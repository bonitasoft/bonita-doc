...
WARNING: If you use different users for the engine data database and for the business data database, then you must grant the following privileges to both users. 
...
CREATE TABLE and CREATE INDEX privileges are not required for the engine database user after first start in normal use. +
...
+
WARNING: The queries must be done for *both* the bonita engine *and* the business data users. +
...
+
Note that the business database user (the one defined in `bdm.db.user`) grant one additional privilege: CREATE SEQUENCE
+
SQL query >
+
[source,sql]
