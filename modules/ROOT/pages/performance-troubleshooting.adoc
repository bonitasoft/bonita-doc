...
---- 
For Subscription Edition only, this timeout can be increased in the Connector service configuration in `bonita-tenant-community-custom.properties` and `bonita-tenant-sp-custom.properties` (See xref:BonitaBPM_platform_setup.adoc[platform setup]) by increasing the value of the property `bonita.tenant.connector.timeout`.  

Additonally, any connectors longer than 10 seconds will produce a warning log.  
