...
If your organization structure is maintained in LDAP, to avoid duplication of the organization data and its maintenance, you are recommended to use the xref:ldap-synchronizer.adoc[LDAP synchronizer], which is available with the Bonita Performance, Efficiency, and Teamwork editions.
...
If you are using the Bonita Performance, Efficiency, or Teamwork edition, you can xref:processes.adoc[define the actor mapping in Bonita Portal] after a process is deployed. +
