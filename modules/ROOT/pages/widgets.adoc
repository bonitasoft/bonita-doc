...
NOTE: Read xref:widgets.adoc#_table_widget[Table widget] section to know how you can customize display of your data table widget.
...
--
[.symptom]
FileViewer widget does not preview a file (although a point on a supported format)
...
Make a custom widget from the File Viewer widget, and adapt the `controller.isPDF` or `controller.isImage` function to make it return true.
--
