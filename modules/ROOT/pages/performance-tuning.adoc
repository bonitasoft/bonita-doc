...
==== Garbage Collector
We recommend to update the Garbage collector parameters to use the following ones (`UseParallelGC` has been proven more efficient than `UseParNewGC` in our experience), depending on wether you use Java 8 or Java 11:  +
...
. `maxSize` maximum of records that will be saved by the time tracker before a flush. If the maximum number of records is reached before the scheduled flush, the older ones are discarded. To avoid the loss of information, a number sufficiently big in comparison with `flushIntervalInSeconds` should be chosen
. `flushIntervalInSeconds` the interval between two flushes on the timetracker thread.
...
. `memory.maxSize` maximum amount of records saved in memory. If the maximum number of records is reached before the scheduled flush, the older ones are discarded. To avoid the loss of information, a number sufficiently big in comparison with `flushIntervalInSeconds` should be chosen
