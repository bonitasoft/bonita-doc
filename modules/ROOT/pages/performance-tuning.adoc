...
===== Garbage Collector
We recommend to update the Garbage collector parameters to use the following ones, depending on wether you use Java 8 or java 11:  +

* Edit file `setup/tomcat-templates/setenv.sh`
* In the line: `CATALINA_OPTS=` 
** If you use *java 8* add: `-XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:ParallelGCThreads=10 -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:${CATALINA_BASE}/logs/gc-$(date +%Y_%m_%d-%H_%M).log`
