...
The Bonita Engine component services are completely configurable using the xref:bonita-bpm-platform-setup.adoc[plaform setup tool] to retrieve the current configuration and update it.
...
| Description:
| Manage categories of processes. A category is a way to classify processes. Categories are handled manually by in the Bonita Administrator Application and are visible to all users.
