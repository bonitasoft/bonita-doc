= WildFly bundle
:description: You will find here steps needed to install and configure a WildFly bundle.

You will find here steps needed to install and configure a WildFly bundle.

The WildFly bundle is a zip archive that contains the Red Hat WildFly Java EE application server packaged with Bonita BPM and xref:BonitaBPM_platform_setup.adoc]#platform_setup_tool[Bonita platform setup tool].
...
For advanced server configuration needs: check out xref:BonitaBPM_platform_setup.adoc]#run_bundle_configure[Bundle configuration] to finely tune your WildFly bundle, using templates used by Bonita BPM.
====
...
xref:BonitaBPM_platform_setup.adoc]#configure_tool[Bonita BPM Platform configuration]
====
