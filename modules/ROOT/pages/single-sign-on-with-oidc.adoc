 ...
 ** Some OpenID Connect providers do not support having a pattern of valid redirect URIs. It is the case with Google's for example. With those provider you can use the `state` parameter to pass the target URL (the URL you initially requested and want to be redirected to). In order to do that set the property `save-target-url-in-state` to `true`. When you do that, the redirect_URI will always be the same. Its default value is the `/bonita/keycloak` path on Bonita server, bu you can set a different value using the property `redirect-uri-path`. Make sure this URL is part of the valid redirect URIs in you OIDC provider configuration for the Bonita client. Those properties are specific to Bonita OIDC module and are not supported/documented by Keycloak.
