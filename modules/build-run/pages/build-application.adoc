...
* `-Dinclude.profiles`: `*.xml` by default. Glob pattern expression to select profiles files to include in the application binary.footnote:[Teamwork, Efficiency, Performance and Enterprise editions only.]
...
* `mvn clean package -Penv-MyEnvConfig`: Where `MyEnvConfig` is the name of one of your custom environment configurations. Using this profile ensures that the application is built with all the necessary Maven dependencies included from your  xref:ROOT:managing-dependencies.adoc[Process dependencies].
