...
WARNING: This application archive is not an executable. It can be used with the xref:{bcdDocVersion}@bcd::deployer.adoc[la-deployer] using the deprecated _Platform mode_.
...
[source,shell]
----
$ my-project> mvn clean package -Pbundle <1>
----
<1> `-Pbundle` or `-P bundle` in Windows terminal, activate a Maven profile that trigger Tomcat Bundle build during the package phase of the build lifecycle.
...
[source,shell]
----
$ my-project> mvn clean package -Pdocker <1>
----
<1> `-Pdocker` or `-P docker` in Windows terminal, activate a Maven profile that trigger Docker image build during the package phase of the build lifecycle. Docker must be installed and available in the PATH env variable of your terminal session.
