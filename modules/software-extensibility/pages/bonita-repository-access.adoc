...
Bonita Artifact Repository is a solution to access Maven artifacts and Docker images produced by Ofelia.
...
- Or you set an incorrect password in the `<server></server>` section. If you used maven encryption, it should start and ends with curly braces (i.e. `<password>{MZ8lWg+K9FA9B0qh/RkJgN}</password>`). You also need to https://maven.apache.org/guides/mini/guide-encryption.html#escaping-curly-brace-literals-in-your-password-since-maven-2-2-0[escape curly-brace literals in your password].
...
[.cause]#Cause#
Your credentials are valid but you are not allowed to access what you are asking for.
