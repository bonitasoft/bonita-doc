...
* _Users with Custom information_ is a new actor filter used to assign tasks to users based on xref:ROOT:custom-user-information-in-bonita-bpm-studio.adoc[custom information] (available from 6.3.2).
...
For details of how to apply an actor filter to a task or lane, see xref:ROOT:actors.adoc[Actors].
