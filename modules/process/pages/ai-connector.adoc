...
If parameter is not present, the temperature will not be set in chat context.
|null
|===
...
If not null, the connector will try to read the specified document and send it as an attachment to the user prompt. Format supported are "doc", "docx", "pdf", ... (see https://tika.apache.org/3.1.0/formats.html)
|null
...
The connector expect a list of String (like `List.of("firstName","lastName","address")`.
|null
...
|*categories*
|*true*
a|The list of category used to classify the given document. 

The connector expect a list of String (like `List.of("RIB","ID",...)`.
