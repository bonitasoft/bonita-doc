...
:description: This page explains how to create and test custom actor filters using the provided maven archetype or Bonita Studio.
...
== Create a custom actor filter with Bonita Studio

Bonita Studio allows you to create a connector directly in your Bonita Project. See xref:software-extensibility:extensions-sdk.adoc#servers[more details here].

== Create an independent custom actor filter from the Maven Archetype
...
image::connector-def-xsd-overview.png[Connector definition xsd overview]
...
image::connector-impl-xsd-overview.png[Connector implementation xsd overview]
...
The entry point of the implementation sources must extend the {javadocUrl}/api/{javadocVersion}/org/bonitasoft/engine/filter/AbstractUserFilter.html[__AbstractUserFilter__, window="_blank"] class
or implement {javadocUrl}/api/{javadocVersion}/org/bonitasoft/engine/filter/UserFilter.html[__UserFilter__, window="_blank"] interface.
