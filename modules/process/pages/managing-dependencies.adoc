...
Bonita Studio supports managing environment-specific dependencies via custom Maven profiles. This approach is preferred over directly adding jar dependencies, as it keeps your build configuration flexible and maintainable.

By default, your project uses the active profile (e.g., the "default" profile). You can define additional profiles for specific environments. For example, consider a custom profile called `mylib-stub` that simulates a dependency by referencing a stub library (a fake artifact such as `stub.jar` that does not actually exist).

Below is an extract of a pom.xml that illustrates the default profile along with the `mylib-stub` profile:
...
<profiles>
  ...
  <!-- Custom profile for a stub dependency -->
  <profile>
    <id>mylib-stub</id>
    <dependencies>
      <dependency>
...
[source,bash]
----
mvn clean package -Penv-mylib-stub
----
