...
* Actors: An actor represents the user who will carry out tasks in the pool. xref:ROOT:actors.adoc[Configure the actors] that participate in the process. An actor must be defined at pool level before it can be assigned to a lane or task.
...
* Search: A search key is used in the Bonita Applications to find a specific instance of a process. xref:ROOT:search-index.adoc[Specify the search keys] associated with the process.
...
* Actors: From the actors defined for the pool, xref:ROOT:actors.adoc[specify the actors] assigned to this lane. You can also xref:actors.adoc[configure an actor filter] to make the actor assignment more specific.
...
* Parameters (Enterprise, Performance, Efficiency, and Teamwork editions): xref:ROOT:parameters.adoc[Define the parameters] of the lane. A parameter is like a variable but has a value that is fixed for a deployment of a process.
