...
[WARNING]
====
Starting from Bonita 2024.1-u9, Docker deployments have xref:security:java-security-policy.adoc[Java Security Policy] enabled by default, which blocks system command execution. The *System script connector will not work* when this policy is active. To use this connector, you must disable the policy by setting `PRO_GRADE=false` (not recommended for security reasons).
====
