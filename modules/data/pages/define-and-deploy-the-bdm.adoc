...
The Business Data Model is the definition of the business data that is shared by processes and process-based applications. This page explains how to define the Business Data Model and how to deploy it. After the model is defined, you can use it to xref:ROOT:specify-data-in-a-process-definition.adoc[specify the business data used in a process].

== Business data model (BDM)

Your business data is the information that is shared between your processes and applications. It is defined as a set of business objects in a business data model (BDM). There is one BDM that is used by all processes and applications on the same Bonita platform.
...
You are recommended to use business data instead of process data for any data that has meaning outside of one single process. The business data objects are stored in a database that can be accessed by all applications and processes. There is no need to import data from an external system or write it back after a change. The data is automatically stored persistently. It is simple to specify the business objects relevant to a process or task, using Bonita Studio. You can also access a business object programmatically by its data access object (DAO).
...
* There is only one BDM for the entire Bonita platform, so it must include all the objects needed. Updating a model must be done with care, so that the changes do not cause problems for deployed processes or applications.
...
To create a custom query, open the `bom.xml` file in the Studio and go the `query` tab of the Expression editor to write a query script. You can use one of the default queries as an example. A custom query can be on multiple attributes. Only SELECT queries are supported.
...
When you call a query via the REST API, the relevant count query is automatically called in the background to get the total count, which can be used to calculate the number of pages necessary to retrieve all matching results. The result of the count query is visible in the HTTP response header, in the Content-Range field. See xref:bdm-query-response-formats.adoc#_pagination_and_content_range_header[Pagination and Content-Range Header] for more details.
...
In a production environment, indexes are built when the platform starts or after a new BDM is deployed. In a development environment, indexes are built when Bonita Studio starts and when the Studio BDM wizard is closed.
...
This means that you can have a different BDM in each Studio during development, which enables you to test the BDM before deploying it.
...
[id=generated-java-dependencies]
== Generated Java dependencies

Two maven artifacts are generated from the Business Data Model : *<ProjectId>-bdm-model* and *<ProjectId>-bdm-dao-client*. +
The `version` and `groupId` of those artifacts match the Bonita project version.
It is possible to view and retrieve the `bdm-model` dependency from the BDM editor `Maven artifact properties` section. +
...
Can be used as dependency of a java client application using the xref:api:create-your-first-project-with-the-engine-apis-and-maven.adoc#_bonita_client_apis[bonita-client^] and the {javadocUrl}/api/{javadocVersion}/org/bonitasoft/engine/api/APIClient.html#getDAO(java.lang.Class)[APIClient#getDAO^] factory method.
...
[CAUTION]
It is not supported for a Bonita project to depend on the `bdm-model` from another Bonita project.

It is required to have https://maven.apache.org/install.html[Maven^] and https://adoptium.net/temurin/releases/?version={minimalRequiredJavaVersion}[Java^] installed for the following instructions.
...
[WARNING]
====
Before deploying the business data model, you must xref:ROOT:platform-maintenance-mode.adoc[Activate the maintenance mode], so that the update can be performed without affecting ongoing processes.
====
...
. Log into the Bonita Runtime as the technical administrator.
. Go to the *Platform maintenance* page.
. Click *Activate maintenance mode*.
. Then, go to the *Business Data Model* menu (and, for Efficiency, Performance and Enterprise editions, choose *Definition*).
...
For that, you can use the alternative we provide, by uncommenting the following lines in file `platform_conf/current/tenant_engine/bonita-tenants-custom.xml` using xref:runtime:bonita-platform-setup.adoc#update_platform_conf[setup tool].
...
With this alternative, you should carefully update the database before deploying the new BDM version. Otherwise, there are a number of pitfalls you may fall into.
