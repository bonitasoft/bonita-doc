...
There are two design patterns for loading complex business objects: *lazy* or *eager*. +

* With lazy design pattern, only the parent object instance is loaded at initialization. For loading the children objects instances additional REST API calls should be made. 
...
* With eager design pattern, both parent object instance and children objects instances are loaded on the spot.
...
From a technical point of view: the BDM schema is managed by hibernate. The parent object is retrieved by a SELECT sql request.
With EAGER loading, this request is then "enhanced" by adding an OUTER JOIN statement for each child object marked as eager.
While it guarantees that everything is loaded in a single database request, it can lead to very heavy sql queries. It is especially problematic with more complex bdm schemas and can lead to excessive memory and processing power consumption.

Lazy loading, by contrast, retrieves only the parent object. When accessing one of its child objects, a new SELECT statement will be generated.
In practice, if we take a parent object with N children, if we were to load all of them, it will either generate a single request with N outer joins when the children are marked EAGER, or, N+1 SELECT requests when they are marked lazy.
The tradeoff between those two options is execution time vs memory consumption.

[WARNING]
====
If you are not sure about the final size of the parent object with children, we do strongly recommend to use the default *lazy* loading option. Indeed, the *eager* loading option can have a huge impact on the Bonita Runtime performance, due to the additional data load and maintenance of the information in memory. +
