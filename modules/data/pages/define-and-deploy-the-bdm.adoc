...
[WARNING]
====
If you are not sure about the final size of BDM schema, we do *strongly* recommend to use the default *LAZY* loading design pattern. Indeed, *EAGER* loading design pattern can have a huge impacts on the Bonita Runtime performance, due to the additional data load and information maintenance in memory. +
