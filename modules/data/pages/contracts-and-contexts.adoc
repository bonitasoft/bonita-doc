...
Limitation : there is currently no way to customize which business data or document are public in Community edition. When using an Enterprise edition, you may want to use the xref:ROOT:bdm-access-control.adoc[BDM Access Control] to protect data access.
