...
* Use xref:ROOT:define-and-deploy-the-bdm.adoc[business data] for information that is shared between applications or processes.
...
Note that since 7.0, Business data is now available. For a detailed explanation, see the xref:ROOT:define-and-deploy-the-bdm.adoc[Business Data overview].
...
Note: if you add Business data, only add it at the pool/lane level. See this tutorial as a guide for the next steps: xref:ROOT:define-and-deploy-the-bdm.adoc[How to add Business data to a process].
