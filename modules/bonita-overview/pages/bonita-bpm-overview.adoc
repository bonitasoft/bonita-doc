...
* Bonita Super Administrator Application: the tool used by the technical user to setup the Bonita Runtime environment with organization, Business Data Model, and applications. It also allows to start maintenance mode to, for example, update the Business Data Model.
