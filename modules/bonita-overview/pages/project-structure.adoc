...
|xref:ROOT:connector-archetype.adoc[Dedicated SDK] or in the Studio

|xref:process:actor-filtering.adoc[Actor Filter]
|When the provided actor filter do not cover your requirements
|xref:ROOT:actor-filter-archetype.adoc[Dedicated SDK] or in the Studio
...
|Bonita Admin application and Bonita User Application are provided by Ofelia and can be added or removed from applications depending on their needs.
