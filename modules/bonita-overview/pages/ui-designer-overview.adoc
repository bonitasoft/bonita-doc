...
* Start form design in Bonita Studio from the appropriate xref:ROOT:contracts-and-contexts.adoc[contract].
...
There is a set of xref:ROOT:widgets.adoc[standard widgets]. If these do not meet your needs, you can define a xref:custom-widgets.adoc[custom widget].
...
With the form type set to *UI Designer*, you can select a form from the dropdown list and click the pencil icon to edit it, or you can choose *Create a new form*. This is the way to go to create forms as the Studio will auto-generate a form based on the process instantiation or task execution xref:ROOT:contracts-and-contexts.adoc[contract].
...
You can xref:ROOT:live-update.adoc[update a form in production], by exporting a new version of the form from the Bonita UI designer, importing it into the relevant process in the Bonita Adminsitrator Application, and then mapping it to the relevant task or process start event.
...
See xref:ROOT:variables.adoc#_business_data[Data management] section to know how to use this Business Data Model.
