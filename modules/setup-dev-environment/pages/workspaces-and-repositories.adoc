...
* *I see this type of message in the Studio log, but when I open the URL in my browser, it works fine*: `!MESSAGE An exception occurred during push on URI http://your_remote_git_repository/repository/my_bonita_repo.git: http://your_remote_git_repository/repository/my_bonita_repo.git: 503 Service Unavailable`. You probably connect to the internet through a Proxy and you need to configure Proxy Settings in the xref:ROOT:bonita-bpm-studio-preferences.adoc[Studio preferences].  
