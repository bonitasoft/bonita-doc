...
[CAUTION]
====
A Bonita project is Maven multi-module project. The Project ID is used to derive all submodules `artifactId`. It is not supported to manually change the `groupId`, `artifactId` and `version`. It is mandatory to use the `Edit project information...` dialog from the Studio.
====
