...
[WARNING]
====
Starting with version 2024.1, Java versions lower than Java {minimalRequiredJavaVersion} are no longer supported. +
As a result, some of the dependencies in the projects imported from previous Bonita Studio versions may not be compatible with Java {minimalRequiredJavaVersion} (for example, an old dependency may contain some packages that have been moved into the JDK in Java {minimalRequiredJavaVersion}, which could lead to issues with modules as the same package might be loaded twice). +
This typically leads to the following error: `The package XXX is accessible from more than one module: <unnamed>, YYY`

Please double-check all your *dependencies* in the project exported from a previous Bonita Studio version and *make sure they are compatible with Java {minimalRequiredJavaVersion}* before importing it.
====
...
If you are not using xref:git[Git], you can retrieve the whole content of another studio workspace by exporting all its projects and importing them into another studio.
...
* Basic Git workflow knowledge (https://git-scm.com/book/en/v2/Git-Basics-Getting-a-Git-Repository[Reference article])
