...
An event handler is registered on an event by adding an entry to the appropriate map. The list of handlers registered can be extended in the xref:bonita-platform-setup.adoc[`bonita-tenant-sp-custom.xml`] file:
