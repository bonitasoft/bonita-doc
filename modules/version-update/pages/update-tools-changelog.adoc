...
=== Bonita 2026.2 (11.1) update changes

* Create xref:runtime:admin-delegation.adoc[Task Delegation] tables and indexes (for *Subscription editions only*).
* Add the xref:runtime:admin-delegation.adoc[Task Delegation] rules cache configuration (for *Subscription editions only*).

=== Bug fixes

* [BPA-241] Update to 10.3 fails when recreating PK on queriable_log after tenantId removal.

== 3.9.0 - April 28, 2026 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2026.1 (11.0) version.

=== Bonita 2026.1 (11.0) update changes
...
* Add new xref:data:bdm-query-response-formats.adoc[BDM query response format] property `bonita.runtime.business-data.serialization.standard-shape.enabled` that allows to choose between modern / legacy format. The property is set to `false` to preserve the existing behavior.
* Add PostgreSQL trigger to cleanup large objects on table `temporary_content` when rows are deleted. This prevents orphan large objects in the database.
* Create xref:runtime:bdm-data-retention.adoc[BDM Data Retention] tables and indexes (for *Subscription editions only*).
* Add the xref:runtime:bdm-data-retention.adoc[BDM Data Retention] schedule cron property `bonita.runtime.retention.schedule.cron` (for *Subscription editions only*).

== 3.8.1 - April 10, 2026 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2025.2 (10.4) version.

* [BPA-246] Oracle Update Failure: ORA-00955 on PK_REF_BIZ_DATA_INST during 10.2 → 10.3 Migration

== 3.8.0 - November 4, 2025 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2025.2 (10.4) version.

== 3.7.1 - October 10, 2025 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2025.1 (10.3) version.

* [BPA-185] Update to 10.3 fails creating index 'idx_pending_mapping_deadlock' in case of re-run of the tool

== 3.7.0 - September 30, 2025 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2025.1 (10.3) version.

* [CLEANUP] with multi-tenancy removal, the tool removes tenantId column from many tables. It also renames many Foreign / Primary / Unique keys and indexes. +
Some unused tables are also removed.
* [NAMING] the tool script itself is now named `bonita-update` / `bonita-update.bat` (all editions)
...
* [PERFORMANCE] it also introduces a new option `-i` or `--update-indexes` to fix the indexes of the database without actually updating Bonita to a newer version. It can also be launched several times without risk: it does nothing if not necessary.

== 3.5.0 - May 27th, 2024 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2024.2(10.1) version.

== 3.4.1 - April 11th, 2024 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2024.1(10.0) version.

== 3.3.0 - Nov 13th, 2023 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2023.2(9.0) version.

== 3.2.0 - May 11th, 2023 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2023.1(8.0) version.

== 3.1.0 - October 5th, 2022 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2022.2(7.15) version.

== 3.0.2 - September 20th, 2022 - Bonita Update Tool changes
Bonita Update Tool handles updates from all Bonita 7.10 versions up to Bonita 2021.2(7.14) version.

* RUNTIME-1428 - Update to 7.14 keeps the admin pages in editable state

== 3.0.1 - May 05th, 2022 - Bonita Update Tool delivery
Bonita Update Tool handles updates from all Bonita 7.10 versions till the latest *main* Bonita version.

* RUNTIME-1010 	- Migration to 7.14 via 7.13 nullify CONTENT of an entry in PAGE table on *Oracle*

== 2.55.2 - May 05th, 2022 (Bonita 2021.2(7.13))
