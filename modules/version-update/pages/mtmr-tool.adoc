...
Following the commercial depreciation of multi-tenancy use by customer willing to have separate BDMs, the technical depreciation will be effective with Bonita 2023.1. The technical solution for customers willing to have separate BDMs per applications/processes proposed by Bonitasoft will be multi-runtimes. 


In order to prepare the technical conversion from multi-tenancy to multi-runtime, Bonitasoft developped a dedicated tool, that will have to be run as many times as the number of the tenants to be kept. 


The tool will delete everything not related to the tenant specified in the parameter from the given database, this means you will need to dump your current database and mount one new database dump per tenant in your environment. 

NOTE: The tool will have to be executed for every database dump.
...
The aim is to restore a dump of your production database where you want one of your new Bonita Runtimes to run in the future, and then to run the tool on it.
You will need to do it for each of the tenants you want to keep using.
* This tool works on Bonita Platform Databases in version 7.11, 7.12, 7.13, 7.14 or 7.15.
* During the operation, a Bonita Platform *should NOT* be running on the database.
