= Multi-Tenant to Multi-Runtime conversion tool
...
Multi-tenancy is deprecated in Bonita, and disappears with Bonita 7.16 (2023.1). If you used multi-tenancy, you need to run this conversion tool before updating to 7.16.

image:version-update:mtmr-general-idea-schema.svg[General idea of the conversion tool,width=700]

== Principle of work
As a pre-requisite, you will need to dump your current database and mount one new database dump per tenant in your environment.
...
=== Postgres
The test was done on a generated database, with the following postgres optimizations: xref:runtime:performance-tuning.adoc#postgresql-performance-tuning[how to finely tune performance for PostgreSQL].
