...
image:mtmr-general-idea-schema.svg[General idea of the conversion tool, width=400]

== Overview

Following the commercial depreciation of multi-tenancy use by customers starting with Bonita 2021.1, the technical solution proposed by Ofelia for customers willing to have separate Bonita engine database (Oracle, SQL, postgres, etc.) per applications/processes, will be multi-runtimes starting with Bonita 2023.1.

In order to prepare the technical conversion from multi-tenancy to multi-runtime, Ofelia developed a dedicated tool - Multi-tenant to multi-runtime conversion tool (MT2MR tool) - available for download {cscDownloadUrl}[on Bonita Customer Portal].
