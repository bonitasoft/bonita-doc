...
* Runtime setup folder custom configuration is kept in the database: Custom configurations made and pushed from the setup folder in the previous version are kept in the database. However, to benefit from the changes and improvements made in the product, you may want to compare the files you pull from your database and the target version files available in the setup/platform_conf/initial folder. If your previous version is lower than 7.3, see special cases xref:#lower-7-3[Updating from a Bonita version lower than 7.3] section below. 
...
If the update doens't go through version 7.0.0, 7.2.0 or 7.2.4, no action is required for the BDM.
* If you are updating from a version lower than 7.3, see special cases xref:#lower-7-3[Updating from a Bonita version lower than 7.3] section below. 
...
* Runtime data in _Bonita Home_ (only if you are xref:#lower-7-3[Updating from a version lower than 7.3])
...
Once the update is complete,  xref:#managing-look-feel.adoc[export the default Look & Feel] from the new version,
modify your custom Look & Feel to be compatible with the new definition, and with the  xref:#creating-a-new-look-feel.adoc[recommendations for form footers]. Then xref:#managing-look-feel.adoc[import] your updated custom Look & Feel into Bonita Portal.
...
If you are using an Oracle database, please follow the instructions for xref:#database-configuration.adoc#jdbc-drivers[Oracle driver download].
...
. xref:#database-configuration.adoc[Configure the bundle to use the updated database.] Do not recreate the database and use xref:#bonita-bpm-platform-setup.adoc[the setup tool] of the bonita-target-version. Edit the bonita-target-version/setup/database.properties file to point to the updated database.
...
==== Manual operations 
. Perform a diff between the source version and the target version of tenants/[TENANT_ID]/tenant_portal/compound-permissions-mapping.properties and put the additional lines into the file tenants/[TENANT_ID]/tenant_portal/compound-permissions-mapping-custom.properties
. Perform a diff between the source version and the target version of tenants/[TENANT_ID]/tenant_portal/resources-permissions-mapping.properties and put the additional lines into the file tenants/[TENANT_ID]/tenant_portal/resources-permissions-mapping-custom.properties
...
. Report all the content of the source version of tenants/[TENANT_ID]/tenant_portal/custom-permissions-mapping.properties into the target version.
