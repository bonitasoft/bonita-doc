...
:description: Learn how to download, launch, and set up the Bonita UI Builder environment for development.
...
If Bonita is running on a port other than 8080, you need to override the `BONITA_PORT` xref:ui-builder-docker-installation.adoc#environment-variables[environment variable] in the `bonita-ui-proxy` service within the `docker-compose.yml` file.
