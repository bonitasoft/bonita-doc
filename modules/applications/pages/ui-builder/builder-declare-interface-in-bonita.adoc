...
As you can see on the screenshot above, both UI Designer applications (declared with an `applications` tag) and UI Builder applications (declared with an `applicationLink` tag) can coexist on the application descriptor and therefore on the same runtime
