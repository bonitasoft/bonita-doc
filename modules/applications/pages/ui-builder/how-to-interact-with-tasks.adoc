...
Open your Bonita UI Builder application, go to the `JS` tab to create a new JavaScript object called for example `humanTask`.
...
export default {
  submitCurrentTask(){
    ...
    //run the executeAssignWorkspaceTask query shown in the previous section
    task.run({taskId: ...}).then(response => {
