...
Zoom controls can be enabled per-application in *App Settings > Zoom*. When enabled, end users can zoom in and out when viewing the deployed application for better accessibility.

This feature is disabled by default and must be explicitly enabled for each application.
