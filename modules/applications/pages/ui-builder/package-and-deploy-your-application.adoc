...
You can export all applications with the button `Export applications` in the workspace. Two possibilities are offered:

* *Save into project* option allows to export all applications and save them directly into the Bonita Studio project in the `web_applications` folder.
* *Download as zip* option allows to export all applications and download them as a zip file containing all JSON application files.

[WARNING]
====
When using the *Save into project* option, be sure that a volume is mount for `/opt/appsmith/workspace/app/exports/` in your docker-compose.

You should have a volume declaration in your docker compose file like this:

`- <path-to-your-bonita-project>/web_applications:/opt/appsmith/workspace/app/exports/`

In a Bonita Studio project, the volume should be set as:
`- ${UIB_WEB_APPLICATIONS:-../../../web_applications}:/opt/appsmith/workspace/app/exports/`
====

image:ui-builder/advanced-app/export-all-application.png[Export all applications]
...
When using Bonita Studio, the `web_applications` folder is automatically created in the project folder. You need to store the exported application(s) in this folder.

[NOTE]
====
By default, the location of the `web_applications` folder in a Bonita Studio project is: `<project-workspace-path>/app/.settings/org.bonitasoft.studio.common.repository/uib/web_applications`
====
...
[WARNING]
--
This strategy doesn't set by default the `BONITA_DEV_MODE` to `false`. You need to set it manually in the environment variables in a docker compose file.
--
