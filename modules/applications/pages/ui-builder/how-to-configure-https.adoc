...
[.cause]#Cause#
This issue arises because the incoming requests that are received by Bonita Runtime are HTTP requests and not HTTPS (since the reverse proxy is used for SSL offloading)
