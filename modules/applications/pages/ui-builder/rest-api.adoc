...
3. <<delete-application-by-slug>>
4. <<import-single-application>>
5. <<update-existing-application>>
6. <<import-multiple-applications>>
7. <<export-all-applications>>
...
Delete a single application from the default workspace identified by its slug.
...
|string
|Yes
|The unique slug identifier of the application to delete
|===
...
|403
|FORBIDDEN
|Access denied to the application with the specified slug.

|404
|NOT_FOUND
|No application found with the specified slug.
...
----
{
  ...
  "errorDisplay": "Application deleted successfully"
}
----
...
----
{
  ...
  "data": null,
  "errorDisplay": "Access denied to application with slug 'my-app-slug'"
}
----
...
----
{
  ...
  "data": null,
  "errorDisplay": "Application with slug 'my-app-slug' not found"
}
----
...
----
{
  ...
  "data": null,
  "errorDisplay": "Failed to delete application: <error details>"
}
----

=== Examples

==== Example 1: Delete application successfully

[source,bash]
----
curl -X DELETE 'http://localhost/uib/api/v1/applications/my-dashboard-app' \
  --header 'x-requested-by: Appsmith' \
  --header 'Cookie: X-Bonita-API-Token=abc123; JSESSIONID=def456'
----
...
==== Example 2: Application not found

[source,bash]
----
curl -X DELETE 'http://localhost/uib/api/v1/applications/non-existent-app' \
  --header 'x-requested-by: Appsmith' \
  --header 'Cookie: X-Bonita-API-Token=abc123; JSESSIONID=def456'
----
...
----
{
  ...
  "data": null,
  "errorDisplay": "Application with slug 'non-existent-app' not found"
}
----
...
Updates an existing application from a JSON descriptor file. The application is identified by its `slug` field in the JSON descriptor.
...
|File
|Yes
|JSON file containing the application descriptor
|===
...
* **Format:** Valid Appsmith application descriptor JSON
* **Slug field:** Must contain a `slug` that matches an existing application
...
|400
|BAD_REQUEST
|Invalid file type or malformed JSON.
...
|500
|INTERNAL_SERVER_ERROR
|Server error during update (e.g., database connection failure, update operation failed).
|===
...
**400 BAD_REQUEST - Invalid file type:**
[source,json]
----
{
  "responseMeta": {
...
----
{
  ...
  "data": null,
  "errorDisplay": "Failed to parse JSON descriptor: <error details>"
}
----
...
----
{
  ...
  "data": null,
  "errorDisplay": "Failed to update application with slug 'my-app-slug'"
}
----

=== Behavior Details

. The JSON descriptor is uploaded and parsed
. The `slug` field is extracted from the JSON
. An existing application with that slug is located in the default workspace
. The application is updated with the new JSON content
. The application is automatically published after update
. The descriptor file is updated in the configured `applicationFolderPath` directory
. Returns `UPDATED` status on success

=== Security Considerations

* **Access Control:** Users must have read permissions on the target application. Unauthorized access returns a 403 FORBIDDEN response.

=== Key Differences from Import

[cols="1,2,2"]
|===
|Aspect |Import (POST) |Update (PUT)
...
|Slug handling
|Creates new slug or uses provided
|Must match existing application
...
[source,bash]
----
curl -X PUT 'http://localhost/uib/api/v1/applications' \
  --header 'x-requested-by: Appsmith' \
  --header 'Cookie: X-Bonita-API-Token=abc123; JSESSIONID=def456' \
  -F 'file=@my-updated-application.json'
----
...
curl -X GET 'http://localhost/uib/api/v1/applications/export/all' \
  --header 'x-requested-by: Appsmith' \
  --header 'Cookie: X-Bonita-API-Token=abc123; JSESSIONID=def456' \
  --output applications-export.zip
----

. **Extract and modify the JSON** (add new pages, modify widgets, etc.)
...
curl -X PUT 'http://localhost/uib/api/v1/applications' \
  --header 'x-requested-by: Appsmith' \
  --header 'Cookie: X-Bonita-API-Token=abc123; JSESSIONID=def456' \
  -F 'file=@modified-application.json'
----
