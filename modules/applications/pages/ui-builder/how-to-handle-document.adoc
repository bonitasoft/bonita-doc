...
If you want to select a document from your computer and upload it to your Bonita application, you can use the `FilePicker` widget, and the Bonita's `formFileUpload` API.
...
set the `Text` property to `{{JSON.parse(uploadDocument.data).filename}}`.
Note the `JSON.parse()` call that allow to transform the response (string) into a JSON object.
