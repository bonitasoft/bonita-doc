...
3. Create a new empty Git repository or open an existing one. See the link:https://docs.gitlab.com/ee/user/project/index.html[GitLab documentation] for instructions on creating a repository.
4. Go to the repository's main page, click on the **Code** button, and copy the **SSH** URL.
5. Paste the SSH URL into the **Generate SSH Key** field in the Bonita UI Builder.
6. Click on the **Generate SSH Keys** button to generate unique `ECDSA 256` and `RSA 4096` keys. Select the key type that meets your security requirements and system capabilities.
...
3. Create a new empty git repository if you don't already have one. See the link:https://support.atlassian.com/bitbucket-cloud/docs/create-a-git-repository/[Bitbucket documentation] for details.
