...
3. Create a new Git repository or open an existing empty repository. See the https://docs.gitlab.com/ee/user/project/index.html[GitLab documentation] for instructions on creating a repository.
4. Go to the repository's main page, click on the **Code** button, and copy the **SSH** URL.
5. Paste the SSH URL into the **Generate SSH Key** field in the Bonita UI Builder.
6. Click on the **Generate SSH Keys** button to generate unique `ECDSA 256` and `RSA 4096` keys. Select the key type that meets your security requirements and system capabilities.
...
Your application is now connected to Bitbucket.
====
...
--
[.symptom]
Unable to connect to self-hosted Git providers.

[.symptom-description]
The SSH connection randomly fails. It may start working again after restarting the container, without any apparent changes.

[.cause]#Potential cause#
Docker assigns internal IP addresses dynamically. Some IP ranges (such as `172.18.0.0/16`) may be blocked or incompatible with self-hosted Git providers network or security rules, preventing SSH access.

[.solution]#Possible solution#
Use a dedicated and controlled IP range to avoid conflicts. Two approaches are available:


1. **Manual (to assign a fixed IP to a specific service)**

Set a https://docs.docker.com/reference/compose-file/services/#ipv4_address-ipv6_address[static IP] in `docker-compose.yml`:
...
services:
  bonita-ui-builder:
    ...
    networks:
      mynet:
        ipv4_address: 10.160.1.11

  bonita-ui-proxy:
    [...]
    networks:
      mynet:
        ipv4_address: 10.160.1.12
----

1. **Global (recommended for multi-project environments)**
Configure Docker via the `daemon.json` file:
...
Ask to your network administrator to ensure that the new IP range does not conflict with existing network rules or other services.
====
