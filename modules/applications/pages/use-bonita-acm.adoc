...
* in the Docker image, by adding `-e 'COM_BONITASOFT_ENGINE_PLUGIN_ACM_REGISTER_ACM_HANDLER=true'` in your docker container creation parameters (See xref:runtime:bonita-docker-installation.adoc#_environment_variables[dedicate Docker page] for more docker options)
