...
In the custom widget editor, use the _uiTranslate_ filter or the _ui-translate_ directive to indicate the content to translate.   Then add the strings to the localization.json file of each page, layout and forms using it. See xref:ROOT:custom-widgets.adoc[Custom widgets section] for more information.
